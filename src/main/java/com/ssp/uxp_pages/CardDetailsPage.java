...
public class CardDetailsPage extends LoadableComponent<CardDetailsPage> {
   ...
   * @param screenshot
   */
  public void switchToFrame(boolean screenshot){
    driver.switchTo().frame(frame_sagepay);
    Log.message("Switched to payment frame", driver, extentedReport, screenshot);
  }
  ...
  public void clickbuy(ExtentTest extentReport, boolean Screenshot) throws Exception {
    try {
      WaitUtils.waitForElementPresent(driver, btnBuy, "Make payment button is not present on Payment Hub");
      btnBuy.click();
