...
public class SearchPage extends LoadableComponent<SearchPage> {
  ...
  private String optMarketYN, optMarketOptionsName;
  

  private void setOptMarketYN(HashMap<String,String> testData) throws Exception {
    this.optMarketYN = testData.get("Mark_Pref");
    this.optMarketOptionsName = testData.get("Mark_Pref_Optns");
    if(optMarketYN.equals("")||optMarketOptionsName.equals("")){
      throw new Exception("Please enter value for field(s) '' & '' in testdata sheet");
    }
  }
  ...
  @FindBy(css = "input[name*='PERSONAL'][name*='MOBILENUMBER']  ")
  WebElement cn_mobilePhone;
  
  @FindBy(css = "input[name*='PERSONAL'][name*='WORKNUMBER']  ")
  WebElement cn_businessPhone;
  ...
  @FindBy(css = "div[class*='radio radio-info radio-inline ViewTransDisable'] label[for*='C2__C1__QUE_B5F37DDFDCE76BB21217342']")
  List <WebElement> OptMarketingList;
  
  @FindBy(css = "div[id='C2__C1__FMT_B86E71877193D01B21745'] input[name*='C2__C1__CONTACTCENTRE[1].CONTACT[1].PERSONAL[1].PREFERENCES[1]']")
  List <WebElement> OptMarketingOptns;

  @FindBy(css = "div[id='C2__C1__FMT_B86E71877193D01B21745'] div[class='checkbox checkbox-info checkbox-inline ViewTransDisable'] label")
  List <WebElement> OptMarketingOptnsName;
...
/*  public void clickCreateCustomer(boolean screenShot, ExtentTest extendReport) throws Exception {
  ...
  public void clickCreateCustomer(boolean screenShot, ExtentTest extendReport) throws Exception {
    try {
     WaitUtils.waitForElementPresent(driver, btnCreateCustomer, "CREATE NEW CUSTOMER button is not present");
     WaitUtils.waitForelementToBeClickable(driver, btnCreateCustomer, "CREATE NEW CUSTOMER button is not clickable");
  ...
  public HashMap<String, String> enterCustomerDetails(HashMap<String, String> testData,
    ...
    try {
      ...
      String mobileNumber = testData.get("Mobile Number").toString();
      String businessNumber = testData.get("Business Number").toString();
      ...
      entercn_MobilePhone(mobileNumber);
      Log.message("Mobile Phone : " + mobileNumber, extendReport);
      
      entercn_BusinessPhone(businessNumber);
      Log.message("Business Phone : " + businessNumber, extendReport);
  ...
  public void entercn_PostCode(String cn_postcode) throws Exception {
    try {
      ...
      WaitUtils.waitForelementToBeClickable(driver, cn_postCode, "");
      cn_postCode.click();
      cn_postCode.clear();
      Thread.sleep(2000);
  ...
  /**
   * To enter the customer Mobile phone field.
   * 
   * @param cn_mobilephone
   * @throws Exception
   */
  public void entercn_MobilePhone(String cn_mobilephone) throws Exception {
    try {
      if(cn_mobilephone != "NULL"){
        cn_mobilePhone.clear();
        cn_mobilePhone.sendKeys(cn_mobilephone);
        Log.message("Phone Number is entered " + cn_mobilephone);
      }     
    }
    catch (Exception e) {
      throw new Exception("Phone number is not entered" + e);
  ...
  /**
   * To enter the customer Business phone field.
   * 
   * @param cn_businessphone
   * @throws Exception
   */
  public void entercn_BusinessPhone(String cn_businessphone) throws Exception {
    try {
      if(cn_businessphone != "NULL"){
        cn_businessPhone.clear();
        cn_businessPhone.sendKeys(cn_businessphone);
        Log.message("Phone Number is entered " + cn_businessphone);
      }   
    }
    catch (Exception e) {
      throw new Exception("Phone number is not entered" + e);
  ...
  public CustDashboardPage confirmCreateCustomer(boolean screenShot, ExtentTest extentedReport)
    ...
    WaitUtils.waitForSpinner(driver);
    try {
      WaitUtils.waitForElement(driver, cn_SaveCustomer, 2);
  ...
  public void enterAuthContactDetails(HashMap<?, ?> testdata, ExtentTest extentReport)
    ...
    try {
      ...
      enterFirstName_cc(testdata.get("First Name").toString() + "jheelumnew", extentReport);
      enterLastName_cc(testdata.get("Last Name").toString() + "comp", extentReport);
   ...
   * 
   */
  public void SelectOptMarketing(HashMap<String,String> testdata, WebDriver driver, boolean screenshot) throws Exception {
    int counter=0;
    try {
      setOptMarketYN(testdata);
      GenericUtils.selectRadioBtn(OptMarketingList, OptMarketingList, optMarketYN, driver, extentedReport, screenshot);
      Thread.sleep(2000);
      for(int i=0;i<OptMarketingOptns.size();i++){
        System.out.println(OptMarketingOptnsName.get(i).getText());
        if(OptMarketingOptnsName.get(i).getText().trim().equals(optMarketOptionsName)){
          GenericUtils.selectCheckBox(OptMarketingOptns.get(i), driver, extentedReport, screenshot);
          Log.message("User has successfully selected marketing option: "+optMarketOptionsName, driver, extentedReport, screenshot);
          counter++;
          break;
        }
      }  
      if(counter==0){
        Log.fail("User has not selected marketing option: "+optMarketOptionsName, driver, extentedReport, screenshot);
        Assert.fail();
      }
      //selectRadioButton("fieldset#FS_QUE_05DBD1E701916C229357553>div.radio", "No");
      Log.message("Selected marketing preferences");
    } catch (Exception e) {
      throw new Exception("Unable to select maketing options " + e);
    }
  }
