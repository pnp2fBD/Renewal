...
/**
* Log class consists capturing and printing screenshots,methods for writing log status as pass/fail
* logs,methods to print test case info and messages
* 
 */
public class CucumberLog extends BaseTest {
  ...
  static final String TEST_TITLE_HTML_BEGIN =
      "&emsp;<div class=\"test-title\"> <strong><font size = \"3\" color = \"#000080\">";
  static final String TEST_TITLE_HTML_END =
      "</font> </strong> </div>&emsp;<div><strong>Steps:</strong></div>";

 static final String TEST_COND_HTML_BEGIN =
      "&emsp;<div class=\"test-title\"> <strong><font size = \"3\" color = \"#0000FF\">";
  ...
  static final String SECURITY_MESSAGE_HTML_BEGIN =
      "<br> <div class=\"test-security-message\">&emsp;<strong><font size = \"3\" color = \"#FF4500\">";
  static final String SECURITY_MESSAGE_HTML_END = "</font> </strong> </div> <br>";

  static final String PASS_HTML_BEGIN =
      "<div class=\"test-result\"><br><font color=\"green\"><strong> ";
  ...
  static final String FAIL_HTML_BEGIN =
      "<div class=\"test-result\"><br><font color=\"red\"><strong> ";
  static final String FAIL_HTML_END1 = " </strong></font> ";
  static final String FAIL_HTML_END2 = "</div>&emsp;";

  static final String SKIP_EXCEPTION_HTML_BEGIN =
      "<div class=\"test-result\"><br><font color=\"orange\"><strong> ";
  ...
  static final String EVENT_HTML_BEGIN =
      "<div class=\"test-event\"> <font color=\"maroon\"> <small> &emsp; &emsp;--- ";
  static final String EVENT_HTML_END = "</small> </font> </div>";

  static final String TRACE_HTML_BEGIN =
      "<div class=\"test-event\"> <font color=\"brown\"> <small> &emsp; &emsp;--- ";
  static final String TRACE_HTML_END = "</small> </font> </div>";

  /**
   * Static block clears the screenshot folder if any in the output during every suite execution and
  ...
  static {
    ...
    File screenShotFolder =
        new File(Reporter.getCurrentTestResult().getTestContext().getOutputDirectory());

    screenShotFolderPath =
        screenShotFolder.getParent() + File.separator + "ScreenShot" + File.separator;
    ...
    File securityHTML =
        new File(Reporter.getCurrentTestResult().getTestContext().getOutputDirectory());
    securityHTMLFolderPath =
        securityHTML.getParent() + File.separator + "Security Scan Report" + File.separator;
    ...
    if (params.containsKey("printconsoleoutput")) {
      CucumberLog.printconsoleoutput = Boolean.parseBoolean(params.get("printconsoleoutput"));
    }

  } // static block

  /**
   * takeScreenShot will take the screenshot by sending driver as parameter in the log and puts in
   * the screenshot folder
   * 
   * depends on system variable isTakeScreenShot status, if true it will take the screenshot, else
  ...
  public static String getSecurityScanHTMLHyperLink(String inputFile) {
    String securityLink = "";
    securityLink = "<a href=\"." + File.separator + "Security Scan Report" + File.separator
        + inputFile + "\" target=\"_blank\" >[HTML Scan Report]</a>";
    return securityLink;
  }

  /**
   * addTestRunMachineInfo will get the information of Hub/Node , browser details if executing
   * through Grid
   * 
  ...
  public static void addTestRunMachineInfo(WebDriver driver) {
    ...
    try {
      hub = (Reporter.getCurrentTestResult().getHost() == null)
          ? Inet4Address.getLocalHost().getHostName() : Reporter.getCurrentTestResult().getHost();
    } catch (UnknownHostException e) {
      e.printStackTrace();
    }
    try {
      testMachine = "(Browser: " + ((RemoteWebDriver) driver).getCapabilities().getBrowserName()
          + ", Hub: " + hub + ", Node: "
          + WebDriverFactory.getTestSessionNodeIP(driver).toUpperCase() + ")";
    } catch (Exception e) {
      e.printStackTrace();
    }
    params[0] = testMachine + ", " + ((RemoteWebDriver) driver).getCapabilities().getBrowserName()
        + "_" + ((RemoteWebDriver) driver).getCapabilities().getPlatform();
  ...
  public static void testCaseInfo(String description) {
    ...
    lsLog4j().info("");
    lsLog4j().log(callerClass(), Level.INFO,
        "****             " + description + "             *****", null);
    lsLog4j().info("");

    if (Reporter.getOutput(Reporter.getCurrentTestResult()).toString()
        .contains("<div class=\"test-title\">")) {
      Reporter.log(TEST_TITLE_HTML_BEGIN + description + TEST_TITLE_HTML_END + "&emsp;");
    } else {
      Reporter
          .log(TEST_TITLE_HTML_BEGIN + description + TEST_TITLE_HTML_END + "<!-- Report -->&emsp;");
    }
  }
  ...
  public static ExtentTest testCaseInfo(String description, String testName, String category,
   ...
   lsLog4j().info("");
    ...
    ExtentTest extentedReport = BaseCucumberTest.extent.startTest(testName, description);
  ...
  public static void endTestCase(ExtentTest extentReporterTests) {
    lsLog4j().info("****             " + "-E---N---D-" + "             *****");
    BaseCucumberTest.extent.endTest(extentReporterTests);
  }
   ...
   * @param description test case
   * @param driver to take screenshot
   * @param extentedReport to write in extent report
   * @param takeScreenShot [OPTIONAL] flag to override taking screenshot
   */
   ...
   * @param description test case
   * @param driver to take screenshot
   * @param extentedReport to write in extentent report
   */
  public static void message(String description, WebDriver driver, ExtentTest extentedReport) {
   ...
   * @param description : test case
   * @param driver : to take screenshot
   * @param inputFile : HTML file name
   */
  ...
  /**
   * message print test case description along with hyper link provided to test case status linked
   * to respective screenshot (level=info)
   * 
   * @param driver - webdriver
   * @param description - log message
   * @param screenshotfolderpath - screen shot folder path
   * @param screenshotfileName - screen shot file name
  ...
  public static void message(WebDriver driver, String description, String screenshotfolderpath,
    ...
    FileUtils.copyFile(screenshot, new File(screenShotFolderPath + File.separator + inputFile));
    Reporter.log(MESSAGE_HTML_BEGIN + String.format("%-75s", description).replace(" ", "&nbsp;")
  ...
  /**
   * buildLogMessage print test case description along with hyper link provided to test case status
   * linked to respective screenshot
   * 
   * @param driver - webdriver
   * @param description - log message
   * @param screenshotfolderpath - screen shot folder path
   ...
   * @throws IOException - java IO exception
   */
  public static String buildLogMessage(WebDriver driver, String description,
      String screenshotfolderpath, String screenshotfileName) throws IOException {
    ...
    return MESSAGE_HTML_BEGIN + String.format("%-75s", description).replace(" ", "&nbsp;")
        + "---&emsp;" + getScreenShotHyperLink(inputFile) + MESSAGE_HTML_END;
  ...
  public static void message(String description, String screenshot) throws IOException {
    ...
    try {
      String inputFile = Reporter.getCurrentTestResult().getName() + "_"
          + screenShotIndex.incrementAndGet() + ".png";
      FileUtils.copyFile(new File(screenshot), new File(screenShotFolderPath + inputFile));
      Reporter.log(
          MESSAGE_HTML_BEGIN + description + getScreenShotHyperLink(inputFile) + MESSAGE_HTML_END);
      lsLog4j().log(callerClass(), Level.INFO, description, null);
  ...
  /**
   * event print the page object custom message in the log which can be seen through short cut keys
   * used during debugging (level=info)
   * 
   * @param description test case
   */
  public static void event(String description) {
    ...
    String currDate =
        new SimpleDateFormat("dd MMM HH:mm:ss SSS").format(Calendar.getInstance().getTime());
  ...
  /**
   * event print the page object custom message in the log which can be seen through short cut keys
   * used during debugging along with duration of the particular action (level=debug)
   * 
   * @param description test case
  ...
  public static void pass(String description, WebDriver driver, Boolean... takeScreenShot) {
    ...
    if (finalDecision) {
      String inputFile = takeScreenShot(driver);
      Reporter.log(PASS_HTML_BEGIN + description + "&emsp;" + getScreenShotHyperLink(inputFile)
          + PASS_HTML_BEGIN);
    } else
  ...
  public static void pass(String description, WebDriver driver) {
    if (configProperty.getProperty("isTakeScreenShot") != null
      ...
      String inputFile = takeScreenShot(driver);
      Reporter.log(PASS_HTML_BEGIN + description + PASS_HTML_END1
          + getScreenShotHyperLink(inputFile) + PASS_HTML_END2);
    } else {
      Reporter.log(PASS_HTML_BEGIN + description + PASS_HTML_END1 + PASS_HTML_END2);
    }
    lsLog4j().log(callerClass(), Level.INFO, description, null);
  ...
  /**
   * failsoft print test case step failure as fail with screenshot and let execution continue
   * (level=error)
   * 
   * @param description custom message in the test case
   * @param driver to take screenshot
  ...
  public static String buildfailsoftMessage(String description, WebDriver driver) {
    ...
    String inputFile = takeScreenShot(driver);
    return "<div class=\"test-result\">&emsp; <font color=\"red\"><strong>" + description
        + " </strong> </font>" + getScreenShotHyperLink(inputFile) + "</div>&emsp;";
  }
   ...
   * @param driver to take screenshot
   * @throws Exception - java/selenium exception
   */
   ...
   * @throws Exception - java/selenium exception
   */
  public static void exception(Exception e, WebDriver driver, ExtentTest extentedReport)
      throws Exception {
    ...
    if (e instanceof SkipException) {
      Reporter.log(SKIP_EXCEPTION_HTML_BEGIN + eMessage + SKIP_HTML_END1
          + getScreenShotHyperLink(inputFile) + SKIP_HTML_END2);
      extentedReport.log(LogStatus.FAIL, eMessage + "&emsp;" + getScreenShotHyperLink(inputFile));
      // extentedReport.log(LogStatus.SKIP, eMessage +
      // extentedReport.addScreenCapture(screenShotFolderPath
      // +inputFile));
    } else {
      Reporter.log(FAIL_HTML_BEGIN + eMessage + FAIL_HTML_END1 + getScreenShotHyperLink(inputFile)
          + FAIL_HTML_END2);
      extentedReport.log(LogStatus.FAIL, eMessage + "&emsp;" + getScreenShotHyperLink(inputFile));
  ...
  /**
   * Asserts that a condition is true or false, depends upon the status. Then it will print the
   * verified message if status is true, else stop the script and print the failed message
   * 
 ...
 public static void assertThat(boolean status, String passmsg, String failmsg) {
    if (!status) {
      CucumberLog.fail(failmsg);
    } else {
      CucumberLog.message(passmsg);
    }
  }

  /**
   * Asserts that a condition is true or false, depends upon the status. Then it will print the
   * verified message with screen shot if status is true, else stop the script and print the failed
   * message with screen shot
   * 
   * @param status - boolean or expression returning boolean
   * @param passmsg -message to be logged when assert status is true
   * @param failmsg -message to be logged when assert status is false
   * @param driver - WebDriver, using this driver will taking the screen shot and mapping to log
   *        report
  */
  public static void assertThat(boolean status, String passmsg, String failmsg, WebDriver driver) {
    if (!status) {
      CucumberLog.fail(failmsg, driver);
    } else {
      CucumberLog.message(passmsg, driver);
    }
  }
  ...
  /**
   * Asserts that a condition is true or false, depends upon the status. Then it will print the
   * verified message if status is true, else print the failed message in red color and continue the
   * next step(not stopping/breaking the test script)
   * 
   * @param status - boolean or expression returning boolean
   * @param passmsg -message to be logged when assert status is true
   * @param failmsg -message to be logged when assert status is false
  ...
  /**
   * Asserts that a condition is true or false, depends upon the status. Then it will print the
   * verified message with screen shot if status is true, else print the failed message in red color
   * with screen shot and continue the next step(not stopping/breaking the test script)
   * 
   * @param status - boolean or expression returning boolean
   * @param passmsg -message to be logged when assert status is true
   * @param failmsg -message to be logged when assert status is false
  ...
  public static void softAssertThat(boolean status, String passmsg, String failmsg,
      WebDriver driver) {
    if (!status) {
      CucumberLog.failsoft(failmsg, driver);
    } else {
      CucumberLog.message(passmsg, driver);
    }
  }
  ...
  */
  public static void softAssertThat(boolean status, String passmsg, String failmsg,
    ...
    if (finalDecision) {
      inputFile = takeScreenShot(driver);
      getScreenShotHyperLink(screenShotFolderPath + inputFile);
      Reporter.log(FAIL_HTML_BEGIN + passmsg + "&emsp;" + getScreenShotHyperLink(inputFile)
          + FAIL_HTML_END1 + FAIL_HTML_END2);
    } else {
      Reporter.log(FAIL_HTML_BEGIN + failmsg + "&emsp;" + getScreenShotHyperLink(inputFile)
          + FAIL_HTML_END1 + FAIL_HTML_END2);
