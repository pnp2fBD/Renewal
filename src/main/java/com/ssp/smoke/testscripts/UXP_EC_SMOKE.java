...
public class UXP_EC_SMOKE extends BaseTest {
  ...
  public  String  topnotification="Some of your policies are due for renewal";
  public  String  countdowntmessage="Your Household Standard Policy is now due for Renewal in No_of_Days days.";
  public  String  startdate="27/09/2018";
  public  String  enddate="26/09/2019";
  public  String  prevPrimium="464.49";
  
  public  String  currPremium="464.49";
  public  String  policynumber="302";
...
//          driver, extentedReport, true);
  ...
  public void TC_004(String browser) throws Exception {
    ...
    try {
       ...
       * 
       * // check for document generation Log.softAssertThat(
       * custdashboardpage.verifyDocumentGeneration(DocumentnameNB, policyDetails.get("Position"),
       * extentedReport), "Documents are generated for 5 start policy",
       * "Documents not generated for 5 start policy", driver, extentedReport, true);
       * 
       * // check the attached document pdf/email
  ...
  public void TC_006(String browser) throws Exception {
    ...
    try {
       ...
       * /*
       * 
       * // click the document tab custdashboardpage.clickDocuments(true, extentedReport); // check
       * for document generation Log.softAssertThat(
       * custdashboardpage.verifyDocumentGeneration(DocumentnameCNL, policyDetails.get("Position"),
       * extentedReport), "Documents are generated for the policy",
       * "Documents not generated for the policy", driver, extentedReport, true);
       * 
       * // check the attached document pdf/email
       * Log.softAssertThat(custdashboardpage.clickdocViewbtn(policyDetails.get( "policyNo"),
       * extentedReport), "Clicked on document view button and verified documents",
      ...
      /*
       * custdashboardpage.clickDocuments(true, extentedReport); // check for document generation
       * Log.softAssertThat( custdashboardpage.verifyDocumentGeneration(DocumentnameRST,
       * policyDetails.get("Position"), extentedReport), "Documents are generated for the policy",
       * "Documents not generated for the policy", driver, extentedReport, true);
       * 
       * // check the attached document pdf/email
       * Log.softAssertThat(custdashboardpage.clickdocViewbtn(policyDetails.get( "policyNo"),
 ...
 carddetailspage = newquotepage.selectVisacard(extentedReport);
     ...
     // custdashboardpage = newquotepage.confirmPayment(testData, extentedReport);
       ...
       * 
       * // check for document generation Log.softAssertThat(
       * custdashboardpage.verifyDocumentGeneration(DocumentnameMTA, policyDetails.get("Position"),
       * extentedReport), "Documents are generated for MTA",
       * "Documents not generated for MTA policy", driver, extentedReport, true);
       * 
       * // check the attached document pdf/email
       * Log.softAssertThat(custdashboardpage.clickdocViewbtn(policyDetails.get( "policyNo"),
       * extentedReport), "Clicked on document view button and verified documents",
  ...
  public void TC_009(String browser) throws Exception {
    ...
    try {
      ...
      if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
        ...
        Log.fail("Failed - NB 5* Policy Created in " + policyDetails.get("Status").toString()
   ...
   * 
   * @Test(description =
   * "Verify the display of Proceed button & behaviour of proceed button and search message",
   * dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider") public void
   * TC_013(String browser) throws Exception {
   * 
   * String tcId = "TC_013"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId);
   * 
   * String userName = testData.get("UserName"); String password = testData.get("Password"); String
   * description = testData.get("Description"); String brandname = testData.get("Brand Name");
   * 
   * Log.testCaseInfo(description + "<small><b><i>[" + browser + "]</b></i></small>"); ExtentTest
   * extentedReport = addTestInfo(tcId, description);
   * 
   * try { // Navigate to Login Page LoginPage loginPage = new LoginPage(driver,
   * webSite,extentedReport).get(); Log.pass("Engagement Centre Landing Page : " + webSite, driver,
   * extentedReport, true);
   * 
   * // Login to the application HomePage homePage = loginPage.loginToSSP(userName, password, false,
   * extentedReport); Log.softAssertThat(homePage.verifyHomePage(),
   * "Successfully logged into UXP Home Page", "Login failed", driver, extentedReport, true);
   * Log.message("Logged in User id:" + userName + "& Password:" + password, driver,
   * extentedReport); // Click on Take Call link homePage.clickTakeCall(extentedReport);
   * homePage.clickMyBrands(brandname, extentedReport); SearchPage searchPage = new
   * SearchPage(driver, extentedReport).get();
   * Log.softAssertThat(searchPage.uielement.verifyPageElements(Arrays.asList("txtPolicyNumber"),
   * searchPage), "Search page is verified", "Search Page is not verified", driver, extentedReport,
   * true);
   * 
   * // Enter Customer Details searchPage.clickCreateCustomer(true, extentedReport); HashMap<String,
   * String> custDetails = searchPage.enterCustomerDetails(testData, true, extentedReport);
   * 
   ...
   * "Verified FirstName and LastName on Customer Dashboard page",
   * "Not Verified Customer Name on Dashboard page", driver, extentedReport, true);
   * custdashboardpage.clickComplete(extentedReport); Log.softAssertThat(homePage.verifyHomePage(),
   * "Successfully navigated to Home Page", "Failed to navigate to Home Page", driver,
   * extentedReport, true);
   * 
   * homePage.clickTakeCall(extentedReport); homePage.clickMyBrands(brandname, extentedReport);
   * searchPage = new SearchPage(driver, extentedReport).get();
   * Log.softAssertThat(searchPage.uielement.verifyPageElements(Arrays.asList("txtPolicyNumber"),
   * searchPage), "Search Page Verified", "Search Page not Verified", driver, extentedReport, true);
   * 
   ...
   * "Verified the message : " + searchPage.SEARCH_NOPOLICY_MSG, "Failed to Verified the message : "
   * + searchPage.SEARCH_NOPOLICY_MSG, driver, extentedReport, true);
   * 
   * custdashboardpage = searchPage.clickSearchedContactPolicy("contact", "",
   * custDetails.get("First Name"), true, extentedReport); // custdashboardpage =
   * searchPage.click_proceedButton(extentedReport, true); Log.softAssertThat(
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardpage), "Navigated to Customer dashboardpage",
   * "Not navigated to Customer dashboardpage", driver, extentedReport, true);
   * 
   * Log.testCaseResult(extentedReport); } catch (Exception e) { Log.exception(e, driver,
   * extentedReport); } finally { Log.endTestCase(extentedReport); driver.quit(); } }
   * 
   * // 1605
   * 
   * @Test(description = "Validate brand search_Make call and Admin Task", dataProviderClass =
   * DataProviderUtils.class, dataProvider = "ssBVTDataProvider") public void TC_014(String browser)
   * throws Exception {
   * 
   * String tcId = "TC_014"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId);
   * 
   * String userName = testData.get("UserName"); String password = testData.get("Password"); String
   * description = testData.get("Description"); String brandname = testData.get("Brand Name");
   * 
   * Log.testCaseInfo(description + "<small><b><i>[" + browser + "]</b></i></small>"); ExtentTest
   * extentedReport = addTestInfo(tcId, description); try {
   * 
   * // Navigate to Login Page LoginPage loginPage = new LoginPage(driver,
   * webSite,extentedReport).get(); Log.pass("Engagement Centre Landing Page : " + webSite, driver,
   * extentedReport, true);
   * 
   * // Login to the application HomePage homePage = loginPage.loginToSSP(userName, password, false,
   * extentedReport); Log.softAssertThat(homePage.verifyHomePage(),
   * "Successfully logged into Home Page", "Login failed", driver, extentedReport, true);
   * Log.message("Logged in with User id:" + userName + "& Password:" + password, driver,
   * extentedReport);
   * 
   * // Click on Take Call link homePage.clickMakeCall(extentedReport);
   * Log.softAssertThat(homePage.verifyBrandsPopup(), "Brands Popup is opened",
   * "Brands Popup is not opened", driver, extentedReport); homePage.clickMyBrands(brandname,
   * extentedReport); SearchPage searchPage = new SearchPage(driver, extentedReport).get();
   * Log.softAssertThat(searchPage.uielement.verifyPageElements(Arrays.asList("txtPolicyNumber"),
   * searchPage), "Search Page Verified", "Search Page not Verified", driver, extentedReport, true);
   * // Enter Customer Details searchPage.clickCreateCustomer(true, extentedReport); HashMap<String,
   * String> custDetails = searchPage.enterCustomerDetails(testData, true, extentedReport);
   ...
   * searchPage.confirmCreateCustomer(true, extentedReport); Log.softAssertThat(
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardpage), "Customer dashboard page is verified",
   * "Customer dashboard page is not verified", driver, extentedReport, true);
   * custdashboardpage.clickComplete(extentedReport); Log.softAssertThat(homePage.verifyHomePage(),
   * "Successfully navigated to Home Page", "Failed to navigate to Home Page", driver,
   * extentedReport, true); homePage.clickTakeCall(extentedReport);
   * homePage.clickMyBrands(brandname, extentedReport); searchPage = new SearchPage(driver,
   * extentedReport).get(); // Search with Valid user names
   * searchPage.enterLastName(custDetails.get("Last Name"), extentedReport);
   * searchPage.enterFirstName(custDetails.get("First Name"), extentedReport);
   * searchPage.clickSearch(extentedReport); searchPage.click_proceedButton(extentedReport, true);
   * searchPage.clickComplete(extentedReport); homePage = new HomePage(driver).get();
   ...
   * homePage.clickMakeCall(extentedReport); homePage.clickBrandPopupCloseBtn(extentedReport, true);
   * homePage.clickMakeCall(extentedReport); homePage.clickBrandPopupCancelBtn(extentedReport,
   * true); homePage.clickMakeCall(extentedReport); homePage.clickOutsideBrandPopup(extentedReport,
   * true); Log.
   * pass("Make  Call - Close button , Cancel button and outside pop up clicks works as expected",
   * extentedReport);
   * 
   * // Checking Admin task homePage.clickAdminTask(extentedReport);
   * Log.softAssertThat(homePage.verifyBrandsPopup(), "Brands Popup is opened",
   * "Brands Popup is not opened", driver, extentedReport);
   * Log.softAssertThat(homePage.isAllBrandsPresent(), "All Brands displayed",
   * "All Brands not displayed", driver, extentedReport, true); homePage.clickMyBrands(brandname,
   * extentedReport);
   * Log.softAssertThat(searchPage.uielement.verifyPageElements(Arrays.asList("txtPolicyNumber"),
   * searchPage), "Search Page Verified", "Search Page not Verified", driver, extentedReport, true);
   * // Search with Valid user names searchPage.enterLastName(custDetails.get("Last Name"),
   * extentedReport); searchPage.enterFirstName(custDetails.get("First Name"), extentedReport);
   * searchPage.clickSearch(extentedReport); searchPage.click_proceedButton(extentedReport, true);
   * searchPage.clickComplete(extentedReport); homePage = new HomePage(driver).get();
   * Log.softAssertThat(homePage.verifyAgentDashboard(), "Agent Dashboard is verified",
   * "Agent Dashboard is not verified", driver, extentedReport, true); // Admin Task
   * homePage.clickAdminTask(extentedReport); homePage.clickBrandPopupCloseBtn(extentedReport,
   * true); homePage.clickAdminTask(extentedReport);
   * homePage.clickBrandPopupCancelBtn(extentedReport, true);
   * homePage.clickAdminTask(extentedReport); homePage.clickOutsideBrandPopup(extentedReport, true);
   * Log.
   * pass("Admin Task - Close button , Cancel button and outside pop up clicks works as expected",
   * extentedReport); Log.testCaseResult(extentedReport);
   * 
   * } catch (Exception e) { Log.exception(e, driver, extentedReport); } finally {
   * Log.endTestCase(extentedReport); driver.quit(); } }
   * 
   * // 1606
   * 
   * @Test(description =
   * "To check that the user is able to access Add/Remove Joint Policyholders feature, when performs Mid-Term Adjustment"
   * , dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider") public void
   * TC_015(String browser) throws Exception {
   * 
   * String tcId = "TC_015"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId); // HashMap<String, String> customerDetails1 = new
   * HashMap<String, String>(); HashMap<String, String> customerDetails2 = new HashMap<String,
   * String>();
   * 
   * String userName = testData.get("UserName"); String password = testData.get("Password"); String
   * description = testData.get("Description"); String brandname = testData.get("Brand Name");
   ...
   * 
   * // Navigate to Login Page LoginPage loginPage = new LoginPage(driver,
   * webSite,extentedReport).get(); Log.pass("SIAAS Landing Page : " + webSite, driver,
   * extentedReport, true);
   * 
   * // Login to the application HomePage homePage = loginPage.loginToSSP(userName, password, false,
   * extentedReport); Log.softAssertThat(homePage.verifyHomePage(),
   ...
   * homePage.clickMyBrands(brandname, extentedReport); SearchPage searchPage = new
   * SearchPage(driver, extentedReport).get(); searchPage.clickCreateCustomer(true, extentedReport);
   * testData = searchPage.enterCustomerDetails(testData, true, extentedReport); String
   * mainCustFirstName = testData.get("First Name"); String mainCustLastName =
   * testData.get("Last Name"); // String mainPolicyholderName = mainCustFirstName + " " +
   * mainCustLastName; Log.message("Contact 1 First Name------>" + mainCustFirstName, driver,
   * extentedReport); Log.message("Contact 1 Last Name------>" + mainCustLastName, driver,
   * extentedReport); String jointPolicyName = mainCustFirstName + " " + mainCustLastName;
   * CustDashboardPage custdashboardpage = searchPage.confirmCreateCustomer(false, extentedReport);
   * Log.softAssertThat( custdashboardpage.verifyContactName( testData.get("Title") + " " +
   * mainCustFirstName + " " + mainCustLastName, extentedReport, true),
   * "Verified FirstName and LastName on Customer Dashboard page",
   * "Not Verified Customer Name on Dashboard page", driver, extentedReport, true);
   * 
   * custdashboardpage.clickComplete(extentedReport); homePage.clickTakeCall(extentedReport);
   * homePage.clickMyBrands(brandname, extentedReport); searchPage.clickCreateCustomer(true,
   * extentedReport); customerDetails2 = searchPage.enterCustomerDetails(testData, true,
   * extentedReport, true); customerDetails2.get("First Name"); customerDetails2.get("Last Name");
   * Log.message("Contact 2 First Name------>" + customerDetails2.get("First Name"), driver,
   * extentedReport); Log.message("Contact 2 Last Name------>" + customerDetails2.get("Last Name"),
   * driver, extentedReport); searchPage.confirmCreateCustomer(false, extentedReport);
   * Log.softAssertThat( custdashboardpage.verifyContactName(testData.get("Title") + " " +
   * customerDetails2.get("First Name") + " " + customerDetails2.get("Last Name"), extentedReport,
   * true), "Verified FirstName and LastName on Customer Dashboard page",
   * "Not Verified Customer Name on Dashboard page", driver, extentedReport, true);
   * 
   * // Create New Quote custdashboardpage.clickNewQuote(true, extentedReport);
   * custdashboardpage.enterQuoteDetails(testData, true, extentedReport); NewQuotePage newquotepage
   * = custdashboardpage.clickContinueQuote(true, extentedReport);
   * 
   * // Create New Quote newquotepage.enterPolicyDetails(testData, true, extentedReport);
   * newquotepage.clickNextOne(extentedReport); newquotepage.enterCustomerDetails(testData, true,
   * Corecover, extentedReport); newquotepage.clickNextTwo(extentedReport);
   * newquotepage.clickView(extentedReport); newquotepage.clickAgree(extentedReport);
   * newquotepage.clickGetQuote(extentedReport);
   * 
   * // Add joint policy holder newquotepage.clickAddPolicyHolderButton(extentedReport);
   ...
   * Log.softAssertThat(newquotepage.verifyPolicyHolderContactName(jointPolicyName, true,
   * extentedReport), jointPolicyName + " - additional policy holder is added", jointPolicyName +
   * " - additional policy holder is not added", driver, extentedReport, true);
   * 
   * newquotepage.clickBuy(extentedReport); newquotepage.selectPayment(testData, true,
   * extentedReport); newquotepage.takePayment(extentedReport); CardDetailsPage carddetailspage =
   ...
   * carddetailspage.selectExpiry(testData, extentedReport, true);
   * carddetailspage.enterVerification(extentedReport, true); carddetailspage.enterName(testData,
   * extentedReport, true); carddetailspage.clickbuy(extentedReport, true);
   * Log.message("Entered Card Details and Navigated to Verificaion page", driver, extentedReport,
   * true);
   * 
   * // Click continue button newquotepage = carddetailspage.clickContinueButton(extentedReport);
   * Log.softAssertThat(newquotepage.verifyPaymentTrasaction(extentedReport),
   * "Payment is successful", "Payment is not successful", driver, extentedReport, true);
   * custdashboardpage = newquotepage.confirmPayment(testData, extentedReport);
   * 
   ...
   * searchPage.clickComplete(extentedReport); homePage.clickTakeCall(extentedReport);
   * homePage.clickMyBrands(brandname, extentedReport); // Search with Valid user names
   * searchPage.enterLastName(mainCustLastName, extentedReport);
   * searchPage.enterFirstName(mainCustFirstName, extentedReport);
   * searchPage.clickSearch(extentedReport);
   * 
   * // Selecting the contact in search page searchPage.clickSearchedContactPolicy("policy", "",
   * mainCustLastName, true, extentedReport);
   * 
   * // Select MTA from drop down custdashboardpage.clickPassVerification(extentedReport, false);
   * custdashboardpage.clickfromManagePolicyDropdown(testData, true, extentedReport);
   * 
   * // Fill up MTa data custdashboardpage.enterMTADetails(testData, extentedReport, true);
   * GetQuotePage getquotepage = new GetQuotePage(driver, extentedReport).get();
   * 
   * // Verify button AddPolicyHolder is accessible Log.softAssertThat(
   * getquotepage.uielement.verifyPageElements(Arrays.asList("btnAddPolicyHolder"), getquotepage),
   ...
   * extentedReport, true);
   * 
   * // Verify bin icon is accessible Log.softAssertThat(getquotepage.verifyBinIcon(jointPolicyName,
   * false, extentedReport),
   ...
   * driver, extentedReport, true); Log.testCaseResult(extentedReport);
   * 
   * } catch (Exception e) { Log.exception(e, driver, extentedReport); } finally {
   * Log.endTestCase(extentedReport); driver.quit(); } }
   * 
   * @Test(description =
   * "To check that the selected policy holder is permanently removed from the list when click on Yes in the confirmation popup window"
   * , dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider") public void
   * TC_016(String browser) throws Exception {
   * 
   * String tcId = "TC_016"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId); // HashMap<String, String> customerDetails1 = new
   * HashMap<String, String>(); HashMap<String, String> customerDetails2 = new HashMap<String,
   * String>();
   * 
   ...
   * testData = searchPage.enterCustomerDetails(testData, true, extentedReport); String
   * mainCustFirstName = testData.get("First Name"); String mainCustLastName =
   * testData.get("Last Name"); Log.message("Contact 1 First Name------>" + mainCustFirstName,
   * driver, extentedReport); Log.message("Contact 1 Last Name------>" + mainCustLastName, driver,
   * extentedReport); String jointPolicyName = mainCustFirstName + " " + mainCustLastName;
   * CustDashboardPage custdashboardpage = searchPage.confirmCreateCustomer(false, extentedReport);
   * Log.softAssertThat( custdashboardpage.verifyContactName( testData.get("Title") + " " +
   * mainCustFirstName + " " + mainCustLastName, extentedReport, true),
   ...
   * homePage.clickMyBrands(brandname, extentedReport);
   * 
   * // Search with Valid user names searchPage.enterLastName(customerDetails2.get("Last Name"),
   * extentedReport); searchPage.enterFirstName(customerDetails2.get("First Name"), extentedReport);
   * searchPage.clickSearch(extentedReport);
   * 
   * // Selecting the contact in search page `
   * searchPage.selectContactFromSearchResult(customerDetails2.get("Last Name"), true,
   * extentedReport); Log.softAssertThat(custdashboardpage.verifyCustomerDashboardPage(),
   * "Customer Dashboard Page is loaded properly", "Customer Dashboard Page is not loaded properly",
   * driver, extentedReport, true);
   * 
   * // Create New Quote // Create New Quote custdashboardpage.clickNewQuote(true, extentedReport);
   * custdashboardpage.enterQuoteDetails(testData, true, extentedReport); NewQuotePage newquotepage
   * = custdashboardpage.clickContinueQuote(true, extentedReport);
   * 
   * // Create New Quote newquotepage.enterPolicyDetails(testData, true, extentedReport);
   ...
   * " - additional policy holder is not added", driver, extentedReport, true);
   * 
   * // To click yes to remove policy newquotepage.clickYesToRemovePolicy(jointPolicyName, true,
   * extentedReport);
   * 
   * Log.softAssertThat(!newquotepage.verifyPolicyHolderContactName(jointPolicyName, true,
   * extentedReport), jointPolicyName + " - is removed using remove Joint Policyholders feature",
   ...
   * 
   * @Test(description =
   * "Check that 'Reverse Transaction' option is available under the 'Manage Policy' sub-section after performing MTA in EC application and verify Modal"
   * , dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider") public void
   * TC_017(String browser) throws Exception {
   * 
   * String tcId = "TC_017"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId);
   * 
   * String userName = testData.get("UserName"); String password = testData.get("Password"); String
   * description = testData.get("Description"); String brandname = testData.get("Brand Name");
   * String Corecover = testData.get("Cover"); String MTAcreationReason =
   * testData.get("MTAdjReason");
   * 
   * Log.testCaseInfo(description + "<small><b><i>[" + browser + "]</b></i></small>"); ExtentTest
   * extentedReport = addTestInfo(tcId, description);
   * 
   ...
   * homePage.clickMyBrands(brandname, extentedReport); SearchPage searchPage = new
   * SearchPage(driver, extentedReport).get(); searchPage.clickCreateCustomer(true, extentedReport);
   * searchPage.enterCustomerDetails(testData, true, extentedReport); // Confirm customer details
   * and create customer CustDashboardPage custdashboardpage =
   * searchPage.confirmCreateCustomer(false, extentedReport); Log.softAssertThat(
   * custdashboardpage.verifyContactName( testData.get("Title") + " " + testData.get("First Name") +
   * " " + testData.get("Last Name"), extentedReport, true),
   * "Verified FirstName and LastName on Customer Dashboard page",
   * "Not Verified Customer Name on Dashboard page", driver, extentedReport, true); // Create New
   * Quote custdashboardpage.clickNewQuote(true, extentedReport);
   * custdashboardpage.enterQuoteDetails(testData, true, extentedReport); NewQuotePage newquotepage
   * = custdashboardpage.clickContinueQuote(false, extentedReport); // Create New Quote
   * Log.softAssertThat(newquotepage.verifyNewQuotePage(true, extentedReport),
   * "Verified NewQuotePage ", "Not verified NewQuotePage", driver, extentedReport, true);
   * newquotepage.enterPolicyDetails(testData, true, extentedReport);
   * newquotepage.clickNextOne(extentedReport); newquotepage.enterCustomerDetails(testData, true,
   * Corecover, extentedReport); newquotepage.clickNextTwo(extentedReport);
   * newquotepage.clickView(extentedReport); newquotepage.clickAgree(extentedReport); // Get and Buy
   * Quote newquotepage.clickGetQuote(extentedReport); // Add the insurance cover items String[]
   * coversToAdd = testData.get("coverToSelect_NB").toString().split(","); for (int i = 0; i <
   * coversToAdd.length; i++) { boolean BoolVal = false; String ins_RowtoInteract =
   * newquotepage.SelectInsuranceItem(coversToAdd[i], true, extentedReport); BoolVal =
   * newquotepage.enterInsCoverDetails(testData, coversToAdd[i], ins_RowtoInteract, true,
   * extentedReport); String[] coverType = coversToAdd[i].split("_"); if (BoolVal != false) {
   * Log.pass(coverType[2] + " " + coverType[0] + " cover done successfully", driver,
   * extentedReport, true); } else { Log.fail("Failed to " + coverType[2] + " " + coverType[0] +
   * " cover", driver, extentedReport, true); } } newquotepage.clickUpgradeNow(extentedReport);
   * Log.softAssertThat(newquotepage.verifyUpgradePolicyTo5Star(), "Policy upgraded to 5 star",
   * "Policy not upgraded to 5 star", driver, extentedReport, true);
   ...
   * newquotepage.selectPayment(testData, true, extentedReport);
   * 
   * // Select card type newquotepage.takePayment(extentedReport); CardDetailsPage carddetailspage =
   * newquotepage.selectVisacard(extentedReport);
   * 
   * // Enter Card Details carddetailspage.enterCardNumber(testData, extentedReport, true);
   * carddetailspage.selectExpiry(testData, extentedReport, true);
   ...
   * "Payment is successful", "Payment is not successful", driver, extentedReport, true);
   * 
   * custdashboardpage = newquotepage.confirmPayment(testData, extentedReport); Log.softAssertThat(
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardpage), "Navigated to Customer dashboard page",
   * "Not navigated to Customer dashboard page", driver, extentedReport, true);
   * 
   ...
   * Log.pass("NB Policy Created Successfully in accepted status", driver, extentedReport, true); }
   * else { Log.fail("Failed - NB Policy Created in " + policyDetails.get("Status").toString() +
   * " status", driver, extentedReport, true); } Log.message("Policy Number ====>" +
   * policyDetails.get("policyNo"), driver, extentedReport);
   ...
   * newquotepage.clickMidTermContinue(extentedReport); GetQuotePage getQuotePage = new
   * GetQuotePage(driver, extentedReport).get(); coversToAdd =
   * testData.get("coverToSelect").toString().split(","); for (int i = 0; i < coversToAdd.length;
   * i++) { boolean BoolVal = false; String ins_RowtoInteract =
   * newquotepage.SelectInsuranceItem(coversToAdd[i], true, extentedReport); BoolVal =
   * newquotepage.enterInsCoverDetails(testData, coversToAdd[i], ins_RowtoInteract, true,
   * extentedReport); String[] coverType = coversToAdd[i].split("_"); if (BoolVal != false) {
   * Log.pass(coverType[2] + " " + coverType[0] + " cover done successfully", driver,
   * extentedReport, true); } else { Log.fail("Failed to " + coverType[2] + " " + coverType[0] +
   * " cover", driver, extentedReport, true); } } getQuotePage.clickReCalculate(extentedReport);
   * getQuotePage.clickBuy(extentedReport);
   * getQuotePage.selectAvalilableAdjReasons(testData.get("AdjustmentReason"), extentedReport,
   * false); getQuotePage.clickAddMTA(extentedReport, false);
   * getQuotePage.clickAddAllMTANextButton(extentedReport, true);
   ...
   * extentedReport, true); carddetailspage.clickbuy(extentedReport, true);
   * Log.message("Entered Card Details and Navigated to Verificaion page", driver, extentedReport,
   * true); getQuotePage = carddetailspage.clickContinueButton_getQuote(extentedReport);
   * Log.softAssertThat(getQuotePage.verifyPaymentTrasaction(extentedReport),
   * "Payment is successful", "Payment is not successful", driver, extentedReport, true);
   * 
   * custdashboardpage = getQuotePage.confirmPayment("MTA", testData, extentedReport);
   ...
   * custdashboardpage), "Navigated to Customer dashboard page",
   * "Not navigated to Customer dashboard page", driver, extentedReport, true);
   * newquotepage.clickManagePolicy(extentedReport); Log.softAssertThat(
   ...
   * , driver, extentedReport, true); newquotepage.clickReverseTransaction(extentedReport);
   * Log.softAssertThat( newquotepage.uielement.verifyPageElements(
   * Arrays.asList("mdlReverseTransaction", "btnConfirmReversal"), newquotepage),
   * "Reverse transaction modal is displayed", "Reverse transaction modal is not displayed", driver,
   * extentedReport, true); Log.testCaseResult(extentedReport); } catch (Exception e) {
   * Log.exception(e, driver, extentedReport); } finally { Log.endTestCase(extentedReport);
   * driver.close(); } }
   * 
   * // 1608
   * 
   * @Test(description =
   * "To check Finance Tab option is available in the Policy Detail section of Customer Dashboard and options will display under Finance Tab. 1. Finance Billing Details 2. Account"
   * , dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider") public void
   * TC_018(String browser) throws Exception {
   * 
   * String tcId = "TC_018"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId);
   * 
   * String userName = testData.get("UserName"); String password = testData.get("Password"); String
   * description = testData.get("Description"); String brandname = testData.get("Brand Name");
   * String tabName = testData.get("TabName"); String Corecover = testData.get("Cover"); String[]
   * sectionsLookingFor = testData.get("SectionsLookingFor").split(";");
   * 
   * Log.testCaseInfo(description + "<small><b><i>[" + browser + "]</b></i></small>"); ExtentTest
   * extentedReport = addTestInfo(tcId, description);
   * 
   ...
   * 
   * // Navigate to Login Page LoginPage loginPage = new LoginPage(driver,
   * webSite,extentedReport).get(); Log.pass("UXP SIAAS Landing Page : " + webSite, driver,
   * extentedReport, true);
   * 
   * // Login to the application HomePage homePage = loginPage.loginToSSP(userName, password, false,
   * extentedReport); Log.softAssertThat(homePage.verifyHomePage(),
   * "Successfully logged into Home Page", "Login failed", driver, extentedReport, true);
   * 
   * // Click on Take Call homePage.clickTakeCall(extentedReport); homePage.clickMyBrands(brandname,
   * extentedReport); SearchPage searchPage = new SearchPage(driver, extentedReport).get();
   * Log.softAssertThat(searchPage.uielement.verifyPageElements(Arrays.asList("txtPolicyNumber"),
   * searchPage), "Search Page Verified", "Search Page not Verified", driver, extentedReport, true);
   * 
   ...
   * 
   * Log.softAssertThat( custdashboardpage.verifyContactName(testData.get("Title") + " " +
   * custDetails.get("First Name") + " " + custDetails.get("Last Name"), extentedReport, true),
   * "Verified FirstName and LastName on Customer Dashboard",
   * "Not Verified Customer Name on Dashboard", driver, extentedReport, true);
   * 
   * // Create New Quote custdashboardpage.clickNewQuote(true, extentedReport);
   ...
   * = custdashboardpage.clickContinueQuote(true, extentedReport);
   * 
   * // Verify Quote Log.softAssertThat(newquotepage.verifyNewQuotePage(true, extentedReport),
   * "Verified NewQuotePage ", "Not verified NewQuotePage", driver, extentedReport, true);
   * 
   * newquotepage.enterPolicyDetails(testData, true, extentedReport);
   * newquotepage.clickNextOne(extentedReport); newquotepage.enterCustomerDetails(testData, true,
   * Corecover, extentedReport); newquotepage.clickNextTwo(extentedReport);
   * newquotepage.clickView(extentedReport); newquotepage.clickAgree(extentedReport); // Get and Buy
   * Quote newquotepage.clickGetQuote(extentedReport);
   * newquotepage.clickAddHomeEmergency(extentedReport);
   * newquotepage.clickUpgradeNow(extentedReport);
   * Log.softAssertThat(newquotepage.verifyUpgradePolicyTo5Star(), "Policy upgraded to 5 star",
   * "Policy not upgraded to 5 star", driver, extentedReport, true);
   * newquotepage.clickReCalculate(extentedReport); newquotepage.clickBuy(extentedReport);
   * newquotepage.selectPayment(testData, true, extentedReport); // Select card type
   * newquotepage.takePayment(extentedReport); CardDetailsPage carddetailspage =
   * newquotepage.selectVisacard(extentedReport);
   * 
   * // Enter Card Details carddetailspage.enterCardNumber(testData, extentedReport, true);
   * carddetailspage.selectExpiry(testData, extentedReport, true);
   ...
   * 
   * // confirm payment custdashboardpage = newquotepage.confirmPayment(testData, extentedReport);
   * Log.softAssertThat(
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardpage), "Customer Dashboard page is verified",
   * "Customer Dashboard page is not verified", driver, extentedReport, true);
   ...
   * "NB Policy Created Successfully in accepted status", "Failed - NB Policy Created in " +
   ...
   * Log.softAssertThat(custDashboardPage.isThisTabAvailableInPolicyDetails("Finance",
   * extentedReport, true), tabName + " Tab Presents", tabName + " Tab not available"); // Click on
   * Finance Tab custDashboardPage.clickTab(tabName, extentedReport, true);
   * 
   * // Check the given sections are available in Finance tab and display
   * Log.softAssertThat(custDashboardPage.showTabSectionsIfAvailable(sectionsLookingFor,
   * extentedReport, false), "Tab sections available", "No tab sctions available", driver,
   * extentedReport, true);
   * 
   * Log.testCaseResult(extentedReport);
   * 
   ...
   * 
   * @Test(description =
   * "To check that the Terms and Conditions can be viewed from 'Manage Policy --> MTA' for an existing contact"
   * , dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", priority =
   * 1) public void TC_019(String browser) throws Exception {
   * 
   * String tcId = "TC_019"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId);
   * 
   * String userName = testData.get("UserName"); String password = testData.get("Password"); String
   * description = testData.get("Description"); String brandname = testData.get("Brand Name");
   ...
   * newquotepage.selectVisacard(extentedReport);
   * 
   * // Step-10: Enter Card Details carddetailspage.enterCardNumber(testData, extentedReport, true);
   * carddetailspage.selectExpiry(testData, extentedReport, true);
   * carddetailspage.enterVerification(extentedReport, true); carddetailspage.enterName(testData,
   * extentedReport, true); carddetailspage.clickbuy(extentedReport, true);
   * Log.pass("Entered Card Details and Navigated to Verificaion page", driver, extentedReport,
   * true); // Click continue button newquotepage =
   ...
   * policyDetails.get("Status").toString() + " status");
   * 
   * // Click on Manage Policy Tab custdashboardpage.selectMTAfromManagePolicyTab(extentedReport,
   * true); custdashboardpage.enterMTADetails(testData, extentedReport, true); GetQuotePage
   * getquotepage = new GetQuotePage(driver, extentedReport).get();
   * getquotepage.clickAcceptanceLink(extentedReport, true); // Verify able to read and edit the
   * terms and conditions
   * Log.softAssertThat(getquotepage.verifyReadTermsAndCondition(extentedReport),
   * "Verified Edit Terms and Conditions", "Not able to verify Edit Terms and Conditions", driver,
   * extentedReport, true);
   * 
   * Log.testCaseResult(extentedReport);
   * 
   * } catch (Exception e) { Log.exception(e, driver, extentedReport); } finally {
   * Log.endTestCase(extentedReport); driver.close(); } }
   * 
   * // 1753 & 1754
   * 
   * @Test(description =
   * "Verify User not able to login with Invalid UserName and Password and valid UserName and Invalid Password and verify password screen"
   * , dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider") public void
   * TC_020(String browser) throws Exception {
   * 
   * String tcId = "TC_020"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId); String[] userName = testData.get("UserName").split(";");
   * String[] password = testData.get("Password").split(";"); String description =
   * testData.get("Description");
   * 
   * Log.testCaseInfo(description + "<small><b><i>[" + browser + "]</b></i></small>"); ExtentTest
   * extentedReport = addTestInfo(tcId, description);
   * 
   ...
   * extentedReport, true);
   * 
   * // Login to the application try { loginPage.loginToSSP(userName[0], password[0], false,
   * extentedReport); } catch (Exception e) {
   * 
   ...
   * driver, extentedReport, true); } // To check the error message
   * Log.softAssertThat(loginPage.verifyErroMessage(), loginPage.ERROR_MSG_LOGIN +
   * " - Error message is displayed ", loginPage.ERROR_MSG_LOGIN +
   * "  Error message is not displayed", driver, extentedReport, true);
   * loginPage.enterUserName(userName[1], extentedReport); loginPage.enterPassword(password[1],
   * extentedReport); loginPage.clickBtnSignIn(extentedReport);
   * Log.softAssertThat(loginPage.verifyErroMessage(), loginPage.ERROR_MSG_LOGIN +
   * " - Error message is displayed ", loginPage.ERROR_MSG_LOGIN +
   * "  Error message is not displayed", driver, extentedReport, true); HomePage homePage =
   * loginPage.loginToSSP(userName[1], password[0], false, extentedReport);
   * Log.softAssertThat(homePage.verifyHomePage(), "Successfully logged into SIAAS Home Page",
   * "Not logged in to Home page", driver, extentedReport, true);
   * 
   * homePage.clickChangePassword(extentedReport); Log.softAssertThat(
   ...
   * 
   * @Test(description =
   * "Check that the user is able to change the payor of the policy, once the quote reaches the summary page"
   * , dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", priority =
   * 1) public void TC_021(String browser) throws Exception {
   * 
   * String tcId = "TC_021"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId);
   * 
   * String userName = testData.get("UserName"); String password = testData.get("Password"); String
   * description = testData.get("Description"); String brandname = testData.get("Brand Name");
   ...
   * extentedReport); Log.message("Contact 2 Last Name------>" + customerDetails2.get("Last Name"),
   * driver, extentedReport); searchPage.confirmCreateCustomer(false, extentedReport);
   * custdashboardpage.clickComplete(extentedReport); // Click on Take Call link
   * homePage.clickTakeCall(extentedReport); homePage.clickMyBrands(brandname, extentedReport);
   * searchPage.enterLastName(mainCustLastName, extentedReport);
   * searchPage.enterFirstName(mainCustFirstName, extentedReport);
   * searchPage.clickSearch(extentedReport); searchPage.clickSearchedContactPolicy("contact", "",
   * mainCustLastName, true, extentedReport); // Create New Quote
   * custdashboardpage.clickNewQuote(true, extentedReport);
   * custdashboardpage.enterQuoteDetails(testData, true, extentedReport); NewQuotePage newquotepage
   * = custdashboardpage.clickContinueQuote(false, extentedReport); // Create New Quote
   * Log.softAssertThat(newquotepage.verifyNewQuotePage(true, extentedReport),
   ...
   * Corecover, extentedReport); newquotepage.clickNextTwo(extentedReport);
   * newquotepage.clickView(extentedReport); newquotepage.clickAgree(extentedReport); // Get and Buy
   * Quote newquotepage.clickGetQuote(extentedReport); newquotepage.clickBuy(extentedReport);
   * newquotepage.selectPaymentPlan(testData.get("Payment Plan"), extentedReport);
   * newquotepage.selectPaymentMethod(testData.get("Payment Method"), extentedReport); // Payor
   * Change newquotepage.clickChangePayorButton(extentedReport, true); String payorname =
   * newquotepage.selectChangePayor(customerDetails2.get("Last Name"),
   ...
   * + payorname, driver, extentedReport, true); Log.testCaseResult(extentedReport); } catch
   * (Exception e) { Log.exception(e, driver, extentedReport); } finally {
   * Log.endTestCase(extentedReport); driver.close(); } }
   * 
   * // 1614
   * 
   * @Test(description = "Verify Reverse transaction after doing Invite Renewal", dataProviderClass
   * = DataProviderUtils.class, dataProvider = "ssBVTDataProvider") public void TC_022(String
   * browser) throws Exception {
   * 
   * // Get the web driver instance String tcId = "TC_022"; final WebDriver driver =
   * WebDriverFactory.get(browser); HashMap<String, String> testData = getTestData(tcId);
   * 
   * String userName = testData.get("UserName"); String password = testData.get("Password"); String
   * description = testData.get("Description"); Log.testCaseInfo(description + "<small><b><i>[" +
   * browser + "]</b></i></small>"); String brandname = testData.get("Brand Name");
   * 
   * ExtentTest extentedReport = addTestInfo(tcId, description); String Corecover =
   * testData.get("Cover");
   * 
   * try {
   * 
   ...
   * searchPage.confirmCreateCustomer(true, extentedReport); Log.softAssertThat(
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardpage), "Customer dashboardpage verified", "Customer dashboardpage not verified",
   * driver, extentedReport, true); Log.softAssertThat(
   * custdashboardpage.verifyContactName(testData.get("Title") + " " + custDetails.get("First Name")
   * + " " + custDetails.get("Last Name"), extentedReport, true),
   * "Verified FirstName and LastName on Customer Dashboard",
   * "Not Verified Customer Name on Dashboard", driver, extentedReport, true);
   * 
   * // Create New Quote custdashboardpage.clickNewQuote(true, extentedReport); // Past date
   * custdashboardpage.enterNewQuotePastDate(5, 1, extentedReport, true);
   * custdashboardpage.selectNewQuoteDetails(testData, extentedReport, true); NewQuotePage
   * newquotepage = custdashboardpage.clickContinueQuote(true, extentedReport);
   * 
   * // Create New Quote Log.softAssertThat(newquotepage.verifyNewQuotePage(true, extentedReport),
   * "Verified NewQuotePage ", "Not verified NewQuotePage", driver, extentedReport, true);
   * newquotepage.enterPolicyDetails(testData, true, extentedReport);
   * newquotepage.clickNextOne(extentedReport); newquotepage.enterCustomerDetails(testData, true,
   * Corecover, extentedReport); newquotepage.clickNextTwo(extentedReport);
   ...
   * newquotepage.clickBuy(extentedReport);
   * 
   * newquotepage.selectPayment(testData, true, extentedReport); // confirm payment
   * custdashboardpage = newquotepage.confirmPayment(testData, extentedReport); Log.softAssertThat(
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardpage), "Navigated to Customer Dashboard page",
   * "Not navigated to Customer Dashboard page", driver, extentedReport, true);
   * 
   * // check for policy status HashMap<String, String> policyDetails =
   * custdashboardpage.getPolicyURN(extentedReport); Log.message("InceptionDate NB:" +
   * policyDetails.get("InceptionDate")); Log.message("ExpiryDate NB:" +
   * policyDetails.get("ExpiryDate"));
   * 
   * if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
   * Log.pass("NB Policy Created Successfully in accepted status", driver, extentedReport, true);
   ...
   * newquotepage.clickNextOne(extentedReport); newquotepage.clickNextTwo(extentedReport);
   * newquotepage.clickView(extentedReport); newquotepage.clickAgree(extentedReport);
   * newquotepage.clickGetQuote(extentedReport); newquotepage.clickQuoteSave("Renew",
   * "Renewal Quote", extentedReport);
   * 
   * Log.softAssertThat(
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   ...
   * extentedReport, true);
   * 
   * Log.softAssertThat(custdashboardpage.verify_reviewStatus("Review Required", extentedReport,
   * true), "Review Required status displayed after saving a renewal quote",
   * "Review Required status was not displayed after saving a renewal quote");
   * 
   * BuyQuotePage buyQuotePage = custdashboardpage.performRenewals_QuoteVariation(extentedReport,
   * true); // Verify Acceptance page is landed
   ...
   * extentedReport, true); // update payment method buyQuotePage.selectPayment(testData, true,
   * extentedReport); custdashboardpage = buyQuotePage.confirmPayment(testData, extentedReport);
   * custdashboardpage.verifyCustomerDashboardPage();
   * 
   ...
   * 
   * Log.softAssertThat( policyDetailsRnwl.get("Status").toString().equalsIgnoreCase("Accepted") &&
   ...
   * 
   * Log.softAssertThat( custdashboardpage.uielement.verifyPageElementsDoNotExist(
   ...
   * , driver, extentedReport, true); String rnwlPremium =
   * custdashboardpage.getPremiums(extentedReport, true);
   ...
   * custdashboardpage.clickConfirmReversalButton(extentedReport); // Validate the policy status and
   * verify the premium String reverseTransPremium = custdashboardpage.getPremiums(extentedReport,
   * true); Log.softAssertThat(reverseTransPremium.equalsIgnoreCase(NBPremium),
   ...
   * ", Renewal Premium:" + rnwlPremium + ", Reversed Premium:" + reverseTransPremium, driver,
   * extentedReport, true);
   * 
   ...
   * && polDet_AfterRevTran.get("ExpiryDate") .equalsIgnoreCase(policyDetails.get("ExpiryDate")),
   ...
   * policyDetailsRnwl.get("ExpiryDate") + ".Reveresed Inception Date:" +
   * polDet_AfterRevTran.get("InceptionDate") + ",Reveresed Expiry Date:" +
   * polDet_AfterRevTran.get("ExpiryDate"), driver, extentedReport, true);
   * 
   * } else { Log.fail("Failed - NB Policy Created in " + policyDetails.get("Status").toString() +
   ...
   * 
   * @Test(description =
   * "Create NB (Add interested party) and do Main search for IP and see whether the search results shows the Policy details"
   * , dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider") public void
   * TC_023(String browser) throws Exception {
   * 
   * String tcId = "TC_023"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId);
   * 
   * String userName = testData.get("UserName"); String password = testData.get("Password"); String
   * description = testData.get("Description"); String firstName = testData.get("First Name");
   * String lastName = testData.get("Last Name"); String CustomerName = testData.get("Title") + " "
   * + firstName + " " + lastName; String addInterestContactType =
   * testData.get("AddInterestContactType"); String brandname = testData.get("Brand Name");
   * Log.testCaseInfo(description + "<small><b><i>[" + browser + "]</b></i></small>");
   * 
   * ExtentTest extentedReport = addTestInfo(tcId, description); String Corecover =
   * testData.get("Cover");
   ...
   * 
   * // Enter Customer Details searchPage.clickCreateCustomer(true, extentedReport); HashMap<String,
   * String> CustomertestData = searchPage.enterCustomerDetails(testData, true, extentedReport);
   * 
   * // Confirm customer details and create customer CustDashboardPage custdashboardPage =
   * searchPage.confirmCreateCustomer(true, extentedReport); Log.softAssertThat(
   * custdashboardPage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardPage), "Customer Dashboard page is verified",
   * "Customer Dashboard page is not verified", driver, extentedReport, true); Log.softAssertThat(
   * custdashboardPage.verifyContactName( CustomertestData.get("Title") + " " +
   * CustomertestData.get("First Name") + " " + CustomertestData.get("Last Name"), extentedReport,
   * true), "Verified Searched FirstName and LastName on Customer Dashboard",
   ...
   * Corecover, extentedReport); newquotepage.clickNextTwo(extentedReport);
   * newquotepage.clickView(extentedReport); newquotepage.clickAgree(extentedReport); // Get and Buy
   * Quote newquotepage.clickGetQuote(extentedReport); newquotepage.checkInterestedParties();
   * Log.softAssertThat(
   * newquotepage.uielement.verifyPageElements(Arrays.asList("noInterestedpartiesMsg"),
   * newquotepage), "No interested parties have been added - text visible",
   ...
   * 
   * newquotepage.clicksearch(extentedReport); newquotepage.clickcreatenewAddparty(extentedReport,
   * true); HashMap<String, String> IPContact = newquotepage.enterCustomerDetails(testData, true,
   * extentedReport); String mainCustFirstName = IPContact.get("First Name"); String
   * mainCustLastName = IPContact.get("Last Name"); newquotepage.confirmCreateCustomer(true,
   * extentedReport);
   * 
   * newquotepage.clickselect(extentedReport, true);
   * 
   ...
   * // newquotepage.addInterestedPartyTypeToDate(extentedReport, true);
   * 
   * newquotepage.addInterestedPartyClickSave(extentedReport, true); String IpNameTochk =
   ...
   * " Added party is visible", IpNameTochk + " Added party is not visible", driver, extentedReport,
   * true);
   * 
   ...
   * Log.pass("NB Policy Created Successfully in accepted status", driver, extentedReport, true);
   * 
   * /// Add search for the existing IP contact custdashboardPage.clickComplete(extentedReport);
   * Log.softAssertThat(homePage.verifyHomePage(), "Successfully logged into UXP Home Page",
   * "Login failed", driver, extentedReport, true);
   * 
   * // Click on Take Call link homePage.clickTakeCall(extentedReport);
   * homePage.clickMyBrands(brandname, extentedReport); Log.softAssertThat(
   * searchPage.uielement.verifyPageElements(Arrays.asList("txtPolicyNumber"), searchPage),
   * "Search Page Verified", "Search Page not Verified", driver, extentedReport, true);
   * 
   * searchPage.enterLastName(mainCustLastName, extentedReport);
   ...
   * custdashboardPage.verifyCustomerDashboardPage(); Log.pass("Navigated to Cutomer Dashboard",
   * driver, extentedReport, true);
   * 
   * // Verifying Customer Details CustomerName = testData.get("Title") + " " + mainCustFirstName +
   * " " + mainCustLastName; Log.softAssertThat(custdashboardPage.verifyContactName(CustomerName,
   * extentedReport, true),
   ...
   * 
   * } else { Log.fail("Failed - NB Policy Created in " + policyDetails.get("Status").toString() +
   * " status", driver, extentedReport, true); } Log.testCaseResult(extentedReport);
   * 
   * } catch (Exception e) { Log.exception(e, driver, extentedReport); } finally {
   * Log.endTestCase(extentedReport); driver.quit(); }
   * 
   ...
   * dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider") public void
   * TC_024(String browser) throws Exception {
   * 
   * String tcId = "TC_024"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId);
   * 
   * String userName = testData.get("UserName"); String password = testData.get("Password"); String
   * description = testData.get("Description");
   * 
   * String brandname = testData.get("Brand Name"); Log.testCaseInfo(description + "<small><b><i>["
   * + browser + "]</b></i></small>");
   * 
   * ExtentTest extentedReport = addTestInfo(tcId, description);
   * 
   ...
   * searchPage.createCorporateContact(testData, extentedReport, true); CustDashboardPage
   * custdashboardPage = searchPage.confirmCreateCustomer_CC(true, extentedReport);
   * 
   * Log.softAssertThat(custdashboardPage.verifyContactName(testData.get("Trade Name"),
   * extentedReport, true), "Verified Business name on Customer Dashboard",
   ...
   * 
   * Log.testCaseResult(extentedReport); } catch (Exception e) { Log.exception(e, driver,
   * extentedReport); } // catch finally { Log.endTestCase(extentedReport); driver.quit(); } //
   * finally
   * 
   * }
   * 
   ...
   * dataProvider = "ssBVTDataProvider") public void TC_026(String browser) throws Exception {
   * 
   * String tcId = "TC_026"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId);
   * 
   * String userName = testData.get("UserName"); String password = testData.get("Password"); String
   * description = testData.get("Description"); String brandname = testData.get("Brand Name");
   * String Corecover = testData.get("Cover");
   * 
   * String adjustmentReason = testData.get("AdjustmentReason"); String paymentPlan =
   * testData.get("ChangePayment Plan"); String paymentMethod =
   * testData.get("ChangePayment Method");
   * 
   * Log.testCaseInfo(description + "<small><b><i>[" + browser + "]</b></i></small>");
   * 
   * ExtentTest extentedReport = addTestInfo(tcId, description); try {
   * 
   * // Navigate to Login Page LoginPage loginPage = new LoginPage(driver,
   * webSite,extentedReport).get(); Log.pass("SIAAS Landing Page : " + webSite, driver,
   * extentedReport, true);
   ...
   * newquotepage.clickView(extentedReport); newquotepage.clickAgree(extentedReport);
   * 
   * newquotepage.clickGetQuote(extentedReport); newquotepage.clickBuy(extentedReport);
   * 
   * newquotepage.selectPayment(testData, true, extentedReport); custdashboardpage =
   * newquotepage.confirmPayment(testData, extentedReport);
   * 
   * Log.softAssertThat(
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardpage), "Navigated to custdashboardpage",
   * "Could not Navigate to custdashboardpage", driver, extentedReport, true);
   * 
   * custdashboardpage.selectBAfromManagePolicyTab(extentedReport, true); BillingAdjustmentPage
   * billingAdjustmentPage = custdashboardpage.enterBADetails(testData, extentedReport, true);
   * 
   * // Selecting adjustment reason billingAdjustmentPage.selectAdjustmentReason(adjustmentReason,
   * extentedReport, true); billingAdjustmentPage.clickNext(extentedReport);
   * 
   * // Selecting Payment method/Plan billingAdjustmentPage.selectPaymentPlan(paymentPlan,
   * extentedReport); billingAdjustmentPage.selectPaymentMethod(paymentMethod, extentedReport,
   * true);
   * 
   * Log.softAssertThat(
   * billingAdjustmentPage.uielement.verifyPageElements(Arrays.asList("panelCardDetailsection"),
   * billingAdjustmentPage), "Card Details panel is displayed",
   * "Card Details panel is not displayed", driver, extentedReport, true);
   * 
   * // Click TakePayment button billingAdjustmentPage.clickTakePaymentBtn(extentedReport);
   * billingAdjustmentPage.selectVisacard(extentedReport);
   * 
   * // Enter Card Details billingAdjustmentPage.enterCardNumber(testData, extentedReport, true);
   ...
   * 
   * // Click continue button billingAdjustmentPage.clickContinueButton(extentedReport);
   * billingAdjustmentPage.clickConfirmBtn(extentedReport);
   * 
   * Log.softAssertThat(
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardpage), "Navigated to customer dashboard page",
   * "Could not navigated to customer dashboard page", driver, extentedReport, true);
   * 
   ...
   * "Billing Adjustment is not completed - Payment method is not changed to - " + paymentMethod,
   * driver, extentedReport, true);
   * 
   * Log.testCaseResult(extentedReport);
   * 
   ...
   * dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider") public void
   * TC_027(String browser) throws Exception { // Get the web driver instance String tcId =
   * "TC_027"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String, String>
   ...
   * = custdashboardpage.clickContinueQuote(true, extentedReport);
   * 
   * // Verify New Quote Log.softAssertThat(newquotepage.verifyNewQuotePage(true, extentedReport),
   * "Verified NewQuotePage ", "Not verified NewQuotePage", driver, extentedReport, true);
   * 
   ...
   * true); // Enter policy details newquotepage.enterPolicyDetails(testData, true, extentedReport);
   * Log.softAssertThat(newquotepage.ECbannertitleCheck(extentedReport),
   * "Data capture page has the banner title as Engagement Centre",
   * "Data capture page failed to have banner title as Engagement Centre", driver, extentedReport,
   * true);
   * 
   * newquotepage.clickNextOne(extentedReport); newquotepage.enterCustomerDetails(testData, true,
   * Corecover, extentedReport); newquotepage.clickNextTwo(extentedReport);
   ...
   * extentedReport, true); newquotepage.clickBuy(extentedReport); // Select Payment type
   * newquotepage.selectPayment(testData, true, extentedReport);
   * Log.softAssertThat(newquotepage.ECbannertitleCheck(extentedReport),
   ...
   * newquotepage.takePayment(extentedReport); CardDetailsPage carddetailspage =
   * newquotepage.selectVisacard(extentedReport); // Enter Card Details
   * carddetailspage.enterCardNumber(testData, extentedReport, true);
   * carddetailspage.selectExpiry(testData, extentedReport, true);
   * carddetailspage.enterVerification(extentedReport, true); carddetailspage.enterName(testData,
   * extentedReport, true); carddetailspage.clickbuy(extentedReport, true);
   ...
   * Log.softAssertThat(
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardpage), "Customer Dashboard page verified", "Customer Dashboard page not verified",
   * driver, extentedReport, true);
   * 
   ...
   * // check for policy status HashMap<String, String> policyDetails =
   * custdashboardpage.getPolicyURN(extentedReport); if
   * (policyDetails.get("Status").equalsIgnoreCase("Accepted")) {
   * 
   * Log.pass("NB Policy Created Successfully in accepted status", driver, extentedReport, true);
   * 
   * // MTA custdashboardpage.clickfromManagePolicyDropdown(testData, true, extentedReport); // Fill
   ...
   * GetQuotePage getquotepage = new GetQuotePage(driver, extentedReport);
   * getquotepage.verifyGetQuotePage(extentedReport); Log.pass("Get quote page loaded successfully",
   * driver, extentedReport, true);
   ...
   * "Get Quote page failed to have banner title as Engagement Centre", driver, extentedReport,
   * true);
   * 
   * // Loop to add items to the existing ins cover String[] coversToAdd =
   * testData.get("coverToSelect_NB").toString().split(",");
   * 
   * for (int i = 0; i < coversToAdd.length; i++) { boolean BoolVal = false; String
   * ins_RowtoInteract = getquotepage.SelectInsuranceItem(coversToAdd[i], true, extentedReport);
   * BoolVal = getquotepage.enterInsCoverDetails(testData, coversToAdd[i], ins_RowtoInteract, true,
   * extentedReport); String[] coverType = coversToAdd[i].split("_"); if (BoolVal != false) {
   * Log.pass(coverType[2] + " " + coverType[0] + "_" + coverType[1] +
   * " cover type done successfully", driver, extentedReport, true); } else { Log.fail("Failed to "
   * + coverType[2] + " " + coverType[0] + "_" + coverType[1] + " cover", driver, extentedReport,
   * true); }
   * 
   * } // Click on recalculate getquotepage.clickReCalculate(extentedReport);
   * 
   * // Click on buy button getquotepage.clickBuy(extentedReport);
   * getquotepage.selectMTAReasonPayment(extentedReport);
   * 
   * // Select card type getquotepage.takePayment(extentedReport); carddetailspage =
   * getquotepage.selectVisacard(extentedReport); // Enter Card Details
   * carddetailspage.enterCardNumber(testData, extentedReport, true);
   * carddetailspage.selectExpiry(testData, extentedReport, true);
   * carddetailspage.enterVerification(extentedReport, true); carddetailspage.enterName(testData,
   * extentedReport, true); carddetailspage.clickbuy(extentedReport, true);
   ...
   * true);
   * 
   * // Click continue button getquotepage =
   * carddetailspage.clickContinueButton_getQuote(extentedReport);
   * Log.softAssertThat(getquotepage.verifyPaymentTrasaction(extentedReport),
   * "Payment was successful", "Payment was not successful", driver, extentedReport, true);
   * 
   ...
   * "Payment page failed to have banner title as Engagement Centre", driver, extentedReport, true);
   * 
   * custdashboardpage = getquotepage.confirmPayment("MTA", testData, extentedReport); // check for
   * policy status policyDetails = custdashboardpage.getPolicyURN(extentedReport);
   * 
   * Log.softAssertThat(getquotepage.ECbannertitleCheck(extentedReport),
   * "Customer dashboard Page has the banner title as Engagement Centre",
   * "Customer dashboard page failed to have banner title as Engagement Centre", driver,
   ...
   * 
   * // STA custdashboardpage.selectSTAfromManagePolicyTab(extentedReport, true);
   * custdashboardpage.enterSTADetails(testData, extentedReport, true); getquotepage =
   * custdashboardpage.verifyGetQuotePage(extentedReport);
   * 
   * Log.softAssertThat(getquotepage.ECbannertitleCheck(extentedReport),
   ...
   * driver, extentedReport, true); getquotepage.clickBuy(extentedReport);
   ...
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardpage), "custdashboardpage Verified", "custdashboardpage not Verified", driver,
   * extentedReport, true); policyDetails = custdashboardpage.getPolicyURN(extentedReport);
   * 
   * Log.softAssertThat(getquotepage.ECbannertitleCheck(extentedReport),
   * "Customer dashboard Page has the banner title as Engagement Centre",
   * "Customer dashboard page failed to have banner title as Engagement Centre", driver,
   ...
   * 
   * } else { Log.
   * fail("Failed to create STA policy in accepted status,it stands in following status:" +
   ...
   * "Complaint page failed to have banner title as Engagement Centre", driver, extentedReport,
   * true);
   * 
   * } else {
   ...
   * 
   * @Test(description =
   * "Check that the user is able to complete the suspension by selecting Accept button",
   * dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider") public void
   * TC_028(String browser) throws Exception {
   * 
   * String tcId = "TC_028"; final WebDriver driver = WebDriverFactory.get(browser); HashMap<String,
   * String> testData = getTestData(tcId);
   * 
   * String userName = testData.get("UserName"); String password = testData.get("Password"); String
   * description = testData.get("Description"); String brandname = testData.get("Brand Name");
   * String Corecover = testData.get("Cover"); String suspendBillingReason =
   * testData.get("SuspendBilling Reason");
   * 
   * Log.testCaseInfo(description + "<small><b><i>[" + browser + "]</b></i></small>"); ExtentTest
   * extentedReport = addTestInfo(tcId, description); try {
   * 
   ...
   * searchPage.confirmCreateCustomer(true, extentedReport); Log.softAssertThat(
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardpage), "Customer dashboard Verified", "Customer dashboard not Verified", driver,
   * extentedReport, true);
   * 
   * // Create New Quote custdashboardpage.clickNewQuote(true, extentedReport);
   * custdashboardpage.enterQuoteDetails(testData, true, extentedReport); NewQuotePage newquotepage
   ...
   * custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
   * custdashboardpage), "Navigated to Customer dashboard page",
   * "Not navigated to Customer dashboard page", driver, extentedReport, true); // Select suspend
   * billing SuspendBillingPage suspendBillingPage =
   ...
   * 
   * // Select Payment Plan & method unsuspendBillingPage.selectPayment(testData, true,
   * extentedReport); custdashboardpage = unsuspendBillingPage.confirmPayment(testData,
  ...
  @Test(description = "SSC Homepage", dataProviderClass = DataProviderUtils.class,
  ...
  public void TC_030(String browser) throws Exception {
    ...
    try {
      ...
      CustomerSelfService cusSelfservice =
          new CustomerSelfService(driver, webSite+web, extentedReport).get();
      ...
      SelfServiceCustomerDashboard Cust_Dashboard = ss_loginPage
          .loginToSSPCustomerSelfService(user_Email, password_SS, extentedReport, false);
      ...
      //Cust_Dashboard.verifyRenewalData(topnotification, countdowntmessage, startdate, enddate, prevPrimium, currPremium, policynumber, false, extentedReport);
