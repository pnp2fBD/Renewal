...
public class WaitUtils {
  ...
  public static ExpectedCondition<Boolean> uiRefspinnerLoad;
  public static int maxPageLoadWait = 180;
  public static int maxElementWait = 30;

  static String cssSpinner = "*[class*='spinning-on-load-bg-table-active']";
  static String UIRSpinner = "div[class*='pre-loader']";
  private static By allSpinners = By.cssSelector(cssSpinner);
  private static By UIRefSpinner = By.cssSelector(UIRSpinner);
...
/*    spinnerLoad = new ExpectedCondition<Boolean>() {
    ...
    uiRefspinnerLoad = new ExpectedCondition<Boolean>() {
      public final Boolean apply(final WebDriver driver) {
        List<WebElement> spinners = driver.findElements(UIRefSpinner);
        for (WebElement spinner : spinners) {
          try {
            if (spinner.isDisplayed()) {
              return false;
        ...
        spinners = driver.findElements(UIRefSpinner);
        for (WebElement spinner : spinners) {
          try {
            if (spinner.isDisplayed()) {
              return false;
...
/*  *//**
  ...
  /**
   * To wait till spinner load and only spinner validation
   * 
   * @param driver -
   */
  public static void waitForOnlySpinner(final WebDriver driver) {
    long startTime = StopWatch.startTime();
    try {
      (new WebDriverWait(driver, 120).pollingEvery(Duration.ofMillis(500))
          .ignoring(NoSuchElementException.class, StaleElementReferenceException.class)
          .withMessage("Realize spinners/page not loading")).until(spinnerLoad);
    } catch (Exception ex) {
      Log.event("Catched spinner load exception");
  ...
  public static void waitForElementPresent(WebDriver driver, WebElement elementTocheck,
      String msg) {
    (new WebDriverWait(driver, maxElementWait).pollingEvery(Duration.ofMillis(200))
        .ignoring(NoSuchElementException.class, StaleElementReferenceException.class)
        .withMessage(msg)).until(ExpectedConditions.visibilityOf(elementTocheck));
  }
  ...
  public static void uiRefwaitForSpinner(final WebDriver driver) {
    long startTime = StopWatch.startTime();
    try {
      Thread.sleep(2000);
      (new WebDriverWait(driver, 60).pollingEvery(500, TimeUnit.MILLISECONDS)
      ...
      (new WebDriverWait(driver, 60).pollingEvery(500, TimeUnit.MILLISECONDS)
          .ignoring(NoSuchElementException.class, StaleElementReferenceException.class)
          .withMessage("Realize spinners/page not loading")).until(uiRefspinnerLoad);
    } catch (Exception ex) {
      Log.event("Catched UIRefesh spinner load exception");
    }
    Log.event("UIRefresh Spinner Load Wait: (Sync)", StopWatch.elapsedTime(startTime));
   ...
   * @param msg : message to print in the log
   */
  public static void waitForinvisiblityofListElement(WebDriver driver, int seconds, String locator,
      String msg) {
    ...
    (new WebDriverWait(driver, 180).pollingEvery(200, TimeUnit.MILLISECONDS)
        .ignoring(NoSuchElementException.class, StaleElementReferenceException.class)
        .withMessage(msg)).until((ExpectedConditions.invisibilityOfAllElements(Elements_toFind)));
  ...
  public static void waitForPageLoad(final WebDriver driver, int maxWait) {
    ...
    FluentWait<WebDriver> wait =
        new WebDriverWait(driver, maxWait).pollingEvery(Duration.ofMillis(500))
  ...
  public static boolean waitForListElement(WebDriver driver, List<WebElement> elements,
    ...
    WebDriverWait wait = new WebDriverWait(driver, maxWait);
    try {
      List<WebElement> waitElementList =
          wait.until(ExpectedConditions.visibilityOfAllElements(elements));
      if (!waitElementList.isEmpty()) {
        statusOfElementToBeReturned = true;
  ...
  public static boolean waitUntilElementDisappear(WebDriver driver, final WebElement element) {
    ...
    WebDriverWait wait =
        (WebDriverWait) new WebDriverWait(driver, WebDriverFactory.maxPageLoadWait);
