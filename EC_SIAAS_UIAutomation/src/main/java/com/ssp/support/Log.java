...
public class Log extends BaseTest {
     ...
     *            mapping to log report
     */
    public static void softAssertThat(boolean status, String passmsg, String failmsg, WebDriver driver, ExtentTest extentedReport, Boolean... takeScreenShot) {
        String inputFile = "";
        boolean finalDecision = true;
        if (takeScreenShot.length > 0 && takeScreenShot[0].equals(Boolean.FALSE)) {
            finalDecision = false;
        ...
        if (finalDecision) {
            inputFile = takeScreenShot(driver);
            getScreenShotHyperLink(screenShotFolderPath +inputFile);
            Reporter.log(FAIL_HTML_BEGIN + passmsg + "&emsp;" + getScreenShotHyperLink(inputFile) + FAIL_HTML_END1 + FAIL_HTML_END2);
        } else {
            Reporter.log(FAIL_HTML_BEGIN + failmsg + "&emsp;" + getScreenShotHyperLink(inputFile) + FAIL_HTML_END1 + FAIL_HTML_END2);

            lsLog4j().log(callerClass(), Level.INFO, failmsg, null);
        }
        if (!status) {
            Log.failsoft(failmsg, driver);
            //extentedReport.log(LogStatus.FAIL, failmsg+ extentedReport.addScreenCapture(screenShotFolderPath + inputFile));
            extentedReport.log(LogStatus.FAIL, failmsg + "&emsp;" + getScreenShotHyperLink(inputFile));
        } else {
            Log.message(passmsg, driver);
            //extentedReport.log(LogStatus.PASS, passmsg+ extentedReport.addScreenCapture(screenShotFolderPath + inputFile));
