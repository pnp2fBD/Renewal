...
public class UXP_EC_1614 extends BaseTest {
	...
	@Test(description = "Check that the Quote description popup window is prompted for quote description (Save/Cancel button), user lands in the the Acceptance page, user is allowed to return to Data Capture and verify the status of Renewed Quote and user should return to datacapture when cancel button clicked in description pop up", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=0)
	public void TC_831_832_833_836_838_835(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.pass("Naviagated to " + testData.get("tabToClick") + " page successfully", driver, extentedReport,
						true);

				Log.softAssertThat(buyQuotePage.clickDataCaptureandVerify(extentedReport, true),
						"Successfully navigated to data catpure page",
						"Failed to navigate to data capture from price presentation page", driver, extentedReport,
						true);
	...
	@Test(description = "Check that the user is allowed to Review/Amend T’s and C’s details", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=3)
	...
	@Test(description = "Verify that user able to update Payment Plan / Method details in policy renewal,Check that the Manage Renewals and Renewal Quote Variation tabs are no longer be available on the Customer Dashboard, when the quote variation is bought and verify the policy status changes to Renewed", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=4)
	...
	@Test(description = "Verify the user is able to update the Change payor details, Check that the warning message is prompted when user performs lapse from the customer dashboard and user should return to the customer dashboard when click cancel in the warning message and verify that quotes are lapsed when clicked continue button", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=5)
	public void TC_844_870_871_872(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				HashMap<String, String> hmQuotedetails = custdashboardpage.getQuoteVariations(extentedReport, true);
				if (hmQuotedetails.get("quoteStatus_0").toString().contentEquals("L")
						&& hmQuotedetails.get("quoteStatus_1").toString().contentEquals("L"))
					Log.pass("Quote variations in customer dashboard page are listed as review required and lapsed",
							extentedReport);
	...
	@Test(description = "Verify the quote variation status when save at price presentation page and Renewal Quote variation tab has Review required and lapse quotes in drop down option and check that user is returned to Price presentation, when clicked on 'Cancel' or 'X' options in the popup window", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=1)
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.softAssertThat(
						...
						"Quote descritpion modal not closed after clicking Cancel button", driver, extentedReport,
						true);
				Log.softAssertThat(buyQuotePage.clickCancelandClosebuttonppPageandVerify("close", extentedReport, true),
						"Quote descritpion modal closed after clicking Close button and user remains in price presentation page",
						"Quote descritpion modal not closed after clicking Close button", driver, extentedReport, true);
	...
	@Test(description = "Check the warning message when lapse the renewal quote variation from Acceptance page and check that renewal quote variation is set as a lapse", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=8)
	...
	@Test(description = "Check that the renewal quote is saved as a new variation with a status of Reviewed, when it is saved from Acceptance page", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=7)
	public void TC_852(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				buyQuotePage.clicksave_LapsebuttoninAcceptancepage("Review Required", "quoteDescrption", "Save",
						extentedReport);
				Log.softAssertThat(
						custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
								custdashboardpage),
	...
	@Test(description = "Check that the user is allowed to update the User Preferences details", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=2)
	...
	@Test(description = "Check that the descriptin popup window is prompted and Check that the 'continue' button is enabled when description entered and check user is returned to Acceptance page, when click on 'cancel' in the popup window and check the reviewed quote status", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=6)
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.softAssertThat(
						...
						"Quote description pop up did not close when cancel button clicked", driver, extentedReport,
						true);
				...
				Log.softAssertThat(
						...
						"Failed to Navigate to custdashboardpage after quote Save", driver, extentedReport, true);
				// Verify the lapsed status of all the quotes
				HashMap<String, String> hmQuotedetails = custdashboardpage.getQuoteVariations(extentedReport, true);
				if (hmQuotedetails.get("quoteStatus_0").toString().contentEquals("R")
						&& hmQuotedetails.get("quoteStatus_1").toString().contentEquals("L"))
	...
	@Test(groups = "Business_Scenario", description = "Verify that user able to complete payment for a lapsed quote", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=13)
	public void BS_014(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				HashMap<String, String> hmQuotedetails = custdashboardpage.getQuoteVariations(extentedReport, true);
				if (hmQuotedetails.get("quoteStatus_0").toString().contentEquals("L")) {
					Log.pass("Quote variations in customer dashboard page are listed as lapsed", extentedReport);
					...
					Log.fail("Quote variations in customer dashboard were not listed listed lapsed", extentedReport);
	...
	@Test(groups = "Business_Scenario", description = "Verify that user able to complete payment for a lapsed quote", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=14)
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				if (hmQuotedetails.get("quoteStatus_0").toString().contentEquals("L")) {
					...
					Log.softAssertThat(custdashboardpage.verifyReinstation(true, extentedReport),
							"Policy Reinstated successfully", "Policy is not reinstated", driver, extentedReport, true);
	...
	@Test(groups = "Business_Scenario", description = "Verify that more renewal variations can be viewed in the quote variations dropdown and review the icons displayed and perform renewal for the most recently created quote variation and the payment type as annual/card", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=16)
	public void BS_020_026_017_022(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				buyQuotePage.clicksave_LapsebuttoninAcceptancepage("Reviewed", renewQuoteDesc, "Save", extentedReport);
				...
				if (hmQuotedetails.get("quoteStatus_0").toString().contentEquals("R")
					...
					Log.fail("Quote variations in customer dashboard were not listed as Review required and lapsed",
							extentedReport);

				// click the recent quote variation and perform transaction
				buyQuotePage = custdashboardpage.clickQuoteVariations(quoteDescription3, extentedReport, true);
	...
	public void createQuoteVariations(WebDriver driver, String quotDesc, CustDashboardPage custdashboardpage,
		...
		buyQuotePage.clicksave_LapsebuttoninAcceptancepage("Review Required", quotDesc, "Save", extentedReport);
		Log.softAssertThat(
				custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"), custdashboardpage),
	...
	@Test(groups = "Business_Scenario", description = "Verify that more renewal variations can be viewed in the quote variations dropdown and review the icons displayed and perform renewal for the original created quote variation(a lapsed quote)", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=15)
	...
	@Test(groups = "Business_Scenario", description = "To check user able view the renewal quote in a contact search and if multiple quotes, all are visible and  view the status of the renewal quote in a contact search", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=9)
	public void BS_001_002(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				custdashboardpage.clickComplete(extentedReport);
				Log.softAssertThat(homePage.verifyHomePage(), "Successfully navigated to UXP Home Page", "Login failed",
						driver, extentedReport, true);
				homePage.clickTakeCall(extentedReport);
				homePage.clickMyBrands(brandname, extentedReport);
	...
	@Test(groups = "Business_Scenario", description = "To check user Accessing a policy within the renewal period will have auto-quote review and auto-quote may be lapsed in manage renewals", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", dependsOnMethods = "BS_001_002",priority=10)
	public void BS_003_004(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(
					custdashboardpage.verifyOptionInRenewalQuoteVariations(quoteDescription1, extentedReport, true),
					"Auto-quote is displayed in quote variations", "Auto-quote is not displayed in quote variations",
			...
			Log.softAssertThat(custdashboardpage.verifyOptionInManageRenewals("Lapse Quote", extentedReport, true),
					"Lapse Quote is displayed in Manage Renewal", "Lapse Quote is not displayed in Manage Renewal",
					driver, extentedReport);
			Log.testCaseResult(extentedReport);
		} catch (Exception e) {
			Log.exception(e, driver, extentedReport);
	...
	@Test(groups = "Business_Scenario", description = "To check user able to select an existing quote and create a quote variation and second is lapsed and user able to view the active and lapsed quote", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", dependsOnMethods = "BS_003_004",priority=11)
	public void BS_005_006_007_008_013(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(buyQuotePage.uielement.verifyPageElements(Arrays.asList("txtTitle"), buyQuotePage),
					"Able to view the active quote variation", "Not able to view the active quote variation", driver,
					extentedReport, true);
			...
			Log.softAssertThat(
					custdashboardpage.verifyStatusForQuoteVariations(quoteDescription1, "L", extentedReport, true),
			...
			Log.softAssertThat(buyQuotePage.uielement.verifyPageElements(Arrays.asList("txtTitle"), buyQuotePage),
					"Able to view the lapsed quote variation", "Not able to view the lapsed quote variation", driver,
					extentedReport, true);
	...
	@Test(groups = "Business_Scenario", description = "To check user create unlimited variations with all previous variations lapsing , Each quote variation includes the: date, variation number, variation description, premium and status notification icon and  all variations  sorted by date or renewal quote with oldest at the bottom and complered renewal quote", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", dependsOnMethods = "BS_005_006_007_008_013",priority=12)
		...
		try {
			...
			Log.softAssertThat(
					(custdashboardpage.verifyStatusForQuoteVariations(quoteDescription3, "R", extentedReport, true)
							&& custdashboardpage.verifyStatusForQuoteVariations(quoteDescription2, "L", extentedReport,
									true)
					...
					"Previous Quotes " + quoteDescription1 + ", " + quoteDescription2 + " are not lapsed", driver,
					extentedReport);

			HashMap<String, String> QuoteVariationsetails = custdashboardpage.getQuoteVariations(extentedReport, true);
			...
			Log.softAssertThat(hashmapSize > 0,
					...
					driver, extentedReport);
			Log.softAssertThat(custdashboardpage.verifyQuoteVariationSortingByDate("desc", extentedReport, true),
					"All variations in the variations dropdown are sorted by date and oldest at the bottom",
					"All variations in the variations dropdown are not sorted by date and oldest at the bottom", driver,
					extentedReport);
			...
			Log.softAssertThat(homePage.verifyHomePage(),
					...
					"User is not able to complete the quote variations and not navigated to Home page. ", driver,
					extentedReport, true);
			Log.testCaseResult(extentedReport);
	...
	@Test(groups = "Business_Scenario", description = "Check that quote variation is not able to save without discription", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=18)
	public void BS_027(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.softAssertThat(
						!(buyQuotePage.uielement.verifyPageElements(Arrays.asList("btnsaveQuoteDesc_acceptance"),
								buyQuotePage)),
	...
	@Test(groups = "Business_Scenario", description = "Verify that Reviewed quote variation have green R in quote variation, Verify that Review Required quote variation have red R in quote variation, Verify that Lapsed quote variation have blue L in quote variation", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=24)
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.softAssertThat(
						...
						"Failed to Navigate to custdashboardpage after quote reviewd", driver, extentedReport, true);
				// Verify the reviewed and lapsed status
				HashMap<String, String> hmQuotedetails = custdashboardpage.getQuoteVariations(extentedReport, true);

				if (hmQuotedetails.get("quoteStatus_0").equals("R") && hmQuotedetails.get("quoteStatus_1").equals("L")
						&& hmQuotedetails.get("quoteStatusCommand_0").equals("status_icon invited")) {
					Log.pass("Renew Quote variations in customer dashboard page are listed as Reviewed and lapsed",
							extentedReport);
				} else {
					Log.fail("Renew Quote variations in customer dashboard page are not listed as Reviewed and lapsed",
							extentedReport);
				}

				buyQuotePage = custdashboardpage.performRenewals_QuoteVariation(extentedReport, true);
				buyQuotePage.clicksave_LapsebuttoninAcceptancepage("Review Required", "Review required Quote variation",
						"Save", extentedReport);
	...
	@Test(groups = "Business_Scenario", description = "Check that lapsed quote variation is not able to delete, Verify that alert message should present when lapse all the variation", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=20)
	public void BS_029_030(String browser) throws Exception {
		...
		String Corecover = testData.get("Cover");
		// String[] review = testData.get("Review").split("\\|");
		String warningMsg = "You are about to lapse this renewal quote variation leaving no active variations against this policy. Do you wish to continue?";
		String brandname = testData.get("Brand Name");
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.softAssertThat(!(GenericUtils.verifyMatchingTextContainsElementFromList(buttons, "delete")),
						"Delete option is not present for lapse quote ", "Delete option is present for lapse quote ",
						driver, extentedReport, true);
				buyQuotePage.clicksave_LapsebuttoninAcceptancepage("Review Required", "Review required Quote variation",
						"Save", extentedReport);
				// buyQuotePage.clicksave_LapsebuttoninAcceptancepage("Review
				...
				if (hmQuotedetails.get("quoteStatus_0").equals("R")) {
					...
					Log.fail("Reviewed quote variation is not present in quote variations", driver, extentedReport,
							true);
				}
			} else {
				Log.fail("Failed - NB Policy Created in " + policyDetails.get("Status").toString()
	...
	@Test(groups = "Business_Scenario", description = " To check Reinstate a cancelled policy, quote then quote variation. Cycle through quote variations until completing. For DD (at renewal and mid-term) and card", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=17)
	...
	@Test(groups = "Business_Scenario", description = "Check that user can able to complete lapsed varition by reinstate it", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=21)
	public void BS_031(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				if (hmQuotedetails.get("quoteStatus_0").equals("L") && hmQuotedetails.get("quoteStatus_1").equals("L")
	...
	@Test(groups = "Business_Scenario", description = "check that user can able to use the variations dropdown to lapse specific variations", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=23)
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				if (hmQuotedetails.get("quoteStatus_0").equals("L")) {
					Log.pass("Able to use the variations dropdown to lapse specific variations", driver, extentedReport,
							true);
				} else {
					Log.fail("Cannot able to use the variations dropdown to lapse specific variations", driver,
							extentedReport, true);
				}
			} else {
				Log.fail("Failed - NB Policy Created in " + policyDetails.get("Status").toString()
	...
	@Test(groups = "Business_Scenario", description = "Verify that two contacts produce exactly the same quote with same conditions and cover", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=19)
	...
	@Test(groups = "Business_Scenario", description = "Verify that user not able to complete quote variation without confirm the gurarantee when using Direct debit", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=22)
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.softAssertThat(
						...
						"Without confim the guarantee able to complete the quote variation ", driver, extentedReport,
						true);
