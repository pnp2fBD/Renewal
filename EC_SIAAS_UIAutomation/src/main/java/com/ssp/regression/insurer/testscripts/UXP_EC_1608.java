...
public class UXP_EC_1608 extends BaseTest {
	...
	@Test(description = "To check Finance Tab option is available in the Policy Detail section of Customer Dashboard", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_239(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(custDashboardPage.isThisTabAvailableInPolicyDetails(tabName, extentedReport, true),
	...
	@Test(description = "To check below options will display under Finance Tab. 1. Finance Billing Details 2. Account", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_240(String browser) throws Exception {
		...
		try {
			...
			// it
			Log.softAssertThat(custDashboardPage.showTabSectionsIfAvailable(sectionsLookingFor, extentedReport, true),
	...
	@Test(description = "To check below options will display under Finance Billing Details screen. Payer, Payment Method, Payment Plan", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_241(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(custDashboardPage.checkFieldsOfFinanceBillingDetails(financialBillingDetailsFields,
					extentedReport, true), "Section Fields available", "Section fields missing");
			Log.testCaseResult(extentedReport);
	...
	@Test(description = "To check below options will display under Account screen. Include Reveals checkbox,  Outstanding checking checkbox, Outstanding Amount, Financial account entry Grid, Take Payment button", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(enabled = false, description = "To check below options will display under 'Financial account entry Grid' screen. 'Entry Date,Due Date,Type,Due,Paid,Balance,Receipts", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	/**
	 * Creates a new customer using the testData given and navigates to
	 * Acceptance tab.
	 * 
	 * @param driver
	 * @param testData
	 * @param upgradePolicyFlag
	 *            - true if need to upgrade policy to 5 star from 3 star; false
	 *            otherwise
	 * @param addCoverFlag
	 *            - true if need to add cover to policy; false otherwise
	...
	/**
	 * Creates a new customer and policy using the testData given and confirms
	 * that the policy is created with 'Accepted' status.
	 * 
	 * @param driver
	 * @param testData
	 * @param upgradePolicyFlag
	 ...
	 * @throws Exception
	 */
	private HashMap<String, String> createACustomerAndPolicy(WebDriver driver, HashMap<String, String> testData,
			boolean upgradePolicyFlag, boolean addCoverFlag, ExtentTest extentedReport) throws Exception {
		try {
			testData = createACustomerAndNavigateToAcceptanceTab(driver, testData, upgradePolicyFlag, addCoverFlag,
					extentedReport);
			...
			Log.softAssertThat(newquotepage.verifyPaymentDetailsAtAcceptanceTab(testData, extentedReport, true),
					"Verified that the payment details at Acceptance tab matches with the details at Cover tab",
					"The payment details at Acceptance tab does not match with the details at Cover tab", driver,
					extentedReport, true);
	 ...
	 * @throws Exception
	 */
	private HashMap<String, String> createTwoCustomers(WebDriver driver, HashMap<String, String> testData,
			ExtentTest extentedReport) throws Exception {
		String userName = testData.get("UserName");
		String password = testData.get("Password");
		String brandname = testData.get("Brand Name");
	...
	@Test(description = "Verify the checkbox selection in the label \"I have spoken directly to the Policy Payor and he/she has confirmed acceptance to making the payment\" when the policy holder is not the policy payor.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", groups = {"Business_Scenario"})
	public void TC_664_665_671_881_883(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(newquotepage.verifyIncludesFee(expectedFee, extentedReport),
					...
					"The 'Includes a fee of' field does have the data as expected", driver, extentedReport, true);
			...
			Log.softAssertThat(newquotepage.verifyPaymentScheduleTable(testData, extentedReport, true),
					...
					"The 'Payment Schedule' table verification is unsuccessful", driver, extentedReport, true);
			
			// Verification of change payor check box absence for default payor
			Log.softAssertThat(
					newquotepage.uielement.verifyPageElementsDoNotExist(Arrays.asList("chkSpokenDirectlyNewPayor"),
							newquotepage),
			...
			Log.softAssertThat(
					newquotepage.uielement.verifyPageElements(Arrays.asList("btnAcceptPayment"), newquotepage),
	...
	@Test(description = "Verify the preferred payment day option by selecting the payment plan which is allowed to change the preferred payment day.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_667(String browser) throws Exception {
		...
		try {
			testData = createACustomerAndNavigateToAcceptanceTab(driver, testData, false, false, extentedReport);
			NewQuotePage newquotepage = new NewQuotePage(driver,extentedReport);
	...
	@Test(description = "Verify 'Add Account Details' section with valid/invalid account number and sort code.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_677_679_680(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(newquotepage.verifyAddAccountDetailsErrorMessage(expectedErrMsg, extentedReport),
					"Verified that the error message is displayed for the invalid sort code given",
					"The error message is not displayed for the invalid sort code given", driver, extentedReport, true);
			...
			Log.softAssertThat(newquotepage.verifyAddAccountDetailsErrorMessage(expectedErrMsg, extentedReport),
					...
					"The error message is not displayed for the invalid account number given", driver, extentedReport,
					true);
			...
			Log.softAssertThat(newquotepage.verifyAddAccountDetailsErrorMessage(expectedErrMsg, extentedReport),
					...
					"The error message is not displayed for the invalid sort code and account number given", driver,
					extentedReport, true);
			Log.softAssertThat(
					newquotepage.uielement.verifyPageElementsDoNotExist(Arrays.asList("btnAcceptPayment"),
							newquotepage),
			...
			Log.softAssertThat(newquotepage.verifyBranchName(expectedBranchName, extentedReport),
					...
					"The branch name field does not display expected branch name and/or the field is editable", driver,
					extentedReport, true);
			...
			newquotepage.selectAccount(validSortCode, expectedBranchName, accountName, validAccNo, extentedReport, true);

			// Account details verification
			Log.softAssertThat(
					newquotepage.verifyAccountSelection(validSortCode, expectedBranchName, accountName, validAccNo,
							extentedReport),
					"Verified that the account selection is successful", "The account selection is not successful",
					driver, extentedReport, true);
	...
	@Test(description = "Verify the existing bank account details listing and the account selection by logging with the user who is having multiple bank accounts.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Verify the policy status, total paid and total balance in Customer Dashboard when the CCA checked the DD guarantee checkbox and clicked on ACCEPT button.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_688_884(String browser) throws Exception {
		...
		try {
			...
			// Account details verification
			newquotepage.selectAccount(sortCode, expectedBranchName, accountName, accNo, extentedReport, true);
			Log.softAssertThat(newquotepage.verifyAccountSelection(sortCode, expectedBranchName, accountName, accNo, extentedReport),
	...
	@Test(description = "Verify the policy status in Customer Dashboard when the CCA checked the DD guarantee checkbox and clicked on CANCEL button.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Verify the policy status in Customer Dashboard when the CCA without checking the DD guarantee checkbox and clicked on CANCEL button.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Verify the policy Status should be Inactive in Customer Dashboard when the CCA processing MTA and backout the transaction", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_692(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(custdashboardpage.verifyTransactionStatus(expectedPosition, expectedTransactionStatus, extentedReport, true),
	...
	@Test(description = "Verify the policy Status should be Qouted Status in Customer Dashboard when the CCA processing MTA clicking on SAVE Quote button.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Verify the ACCEPT button and MTA adjustment radio buttons presence and functionality during Monthly Direct Debit payment mode functionality without/after cliking 'Guarantee...' checkbox after MTA adjustment.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", groups = {"Business_Scenario"})
	public void TC_699_700_707_708_713_716(String browser) throws Exception {
		...
		try {
			...
			for (int i = 0; i < coversToAdd.length; i++) {
				...
				getQuotePage.enterInsCoverDetails(testData, coversToAdd[i], ins_RowtoInteract, true, extentedReport);
			}

			getQuotePage.clickReCalculate(extentedReport);
			testData = getQuotePage.grabPaymentDetailsFromAdjustmentCoverTab(testData, extentedReport, true);
			getQuotePage.clickBuy(extentedReport);
			getQuotePage.selectAvalilableAdjReasons("Add Cover", extentedReport, false);
			getQuotePage.clickAddMTA(extentedReport, false);
			getQuotePage.clickAddAllMTANextButton(extentedReport, true);
			Log.softAssertThat(
					getQuotePage.verifyMTAAdjustmentRadioButtons(false, extentedReport, true),
					"Verified that radio buttons of 'MTA Adjustment' section are displayed as expected",
					"The radio buttons of 'MTA Adjustment' section are not displayed as expected",
					driver, extentedReport, true);
			...
			Log.softAssertThat(getQuotePage.verifyPaymentMethodDisabled(),
					...
					"The 'Payment Method' drop-down is not disabled as expected", driver, extentedReport, true);
			...
			Log.softAssertThat(
					getQuotePage.verifyAcceptButtonPresence(extentedReport),
	...
	@Test(description = "Verify the payment schedule, adjustment radio buttons and save quote functionality while performing return or refund premium MTA transaction.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_709_712_724(String browser) throws Exception {
		...
		try {
			...
			for (int i = 0; i < coversToRemove.length; i++) {
				...
				String ins_RowtoInteract = getQuotePage.SelectInsuranceItem(coversToRemove[i], true, extentedReport);
				isSuccessful = getQuotePage.enterInsCoverDetails(testData, coversToRemove[i], ins_RowtoInteract, true,
						extentedReport);
			...
			Log.softAssertThat(
					getQuotePage.verifyMTAAdjustmentRadioButtons(true, extentedReport, true),
					"Verified that radio buttons of 'MTA Adjustment' section are displayed as expected",
					"The radio buttons of 'MTA Adjustment' section are not displayed as expected",
					driver, extentedReport, true);
			
			// Acceptance tab - Payment details verification
			Log.softAssertThat(getQuotePage.verifyPaymentDetailsAtAdjustmentPaymentAcceptanceTab(testData, extentedReport, true),
					"Verified that the payment details at Acceptance tab matches with the details at Cover tab",
					"The payment details at Acceptance tab does not match with the details at Cover tab", driver,
					extentedReport, true);
	...
	@Test(description = "Verify the functionality clicking on TAKE PAYMENT with the 'Single Bill - Card' option", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Verify the policy Status should be Inactive in Customer Dashboard when the CCA processing MTA and backout the transaction from payment failure page and also verify that the MTA quote can be edited.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", groups = {"Business_Scenario"})
	...
	@Test(description = "Verify the policy Status should be Qouted Status in Customer Dashboard when the CCA processing MTA clicking on SAVE Quote button from payment failure page.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Verify the STA flow by using DD as payment method option selecting monthlt payment type.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_877(String browser) throws Exception {
		...
		try {
			...
			// Acceptance tab - Payment details verification
			Log.softAssertThat(
					getQuotePage.verifyPaymentDetailsAtAdjustmentPaymentAcceptanceTab(testData, extentedReport, true),
					"Verified that the payment details at Acceptance tab matches with the details at Cover tab",
					"The payment details at Acceptance tab does not match with the details at Cover tab", driver,
					extentedReport, true);
			...
			String expectedTransactionStatus = "Active: Debit And Issue Policy";
			Log.softAssertThat(custdashboardpage.verifyTransactionStatus(expectedPosition, expectedTransactionStatus, extentedReport, true),
					"Verified that the STA transaction status is '" + expectedTransactionStatus + "' as expected",
	...
	@Test(description = "Verify the DD payment option with 5 star Defacto rating policy for NB.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_878(String browser) throws Exception {
		...
		try {
			...
			testData = createACustomerAndNavigateToAcceptanceTab(driver, testData, true, false, extentedReport);
			NewQuotePage newquotepage = new NewQuotePage(driver,extentedReport);
	...
	@Test(description = "Verify the DD payment option with 5 star Defacto rating policy for MTA. - Remove cover with Spread over Installments", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Verify the DD payment option with 5 star Defacto rating policy for MTA - Add cover with Single Bill-Card.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Verify the DD payment option with 5 star Defacto rating policy for STA. - Remove cover with Spread over Installments", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_880_A(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(
					getQuotePage.verifyMTAAdjustmentRadioButtons(true, extentedReport, true),
					"Verified that radio buttons of 'STA Adjustment' section are displayed as expected",
					"The radio buttons of 'STA Adjustment' section are not displayed as expected",
					driver, extentedReport, true);
	...
	@Test(description = "Verify the DD payment option with 5 star Defacto rating policy for STA - Add cover with Single Bill-Card.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Verify the display of payment details and installment details in the acceptance page while clicking on BUY button from Dashboard for the quoted policy", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_882(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(
					...
					"The 'Buy' button in GetQuote page is not found. The user might not be on GetQuote page.", driver, extentedReport,
					true);
			...
			Log.softAssertThat(
					...
					"Verified that the payment details at Acceptance tab matches with the details at Cover tab",
					"The payment details at Acceptance tab does not match with the details at Cover tab", driver, extentedReport,
					true);
	...
	@Test(description = "Verify that it is possible to create a STA DD for additional cover", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", groups = {"Business_Scenario"})
	...
	@Test(description = "Verify the policy Status should be Inactive in Customer Dashboard when the CCA processing STA and backout the transaction from payment failure page and also verify that the STA quote can be edited.", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", groups = {"Business_Scenario"})
	...
	@Test(description = "Verify that it is possible to create a MTA for reduction of cover", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", groups = {"Business_Scenario"})
	...
	@Test(description = "Verify that it is possible to pay off outstanding DD instalments in one lump sum", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider", groups = {"Business_Scenario"})
	public void BS_023(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(
					...
					"Card Details panel is displayed", "Card Details panel is not displayed", driver, extentedReport,
					true);
			...
			Log.softAssertThat(custdashboardpage.verifyPaymentMethod(paymentMethod),
					...
					"Billing Adjustment is not completed - Payment method is not changed to - " + paymentMethod, driver,
					extentedReport, true);
