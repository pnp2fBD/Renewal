...
public class UXP_EC_1605 extends BaseTest {
	...
	@Test(description = "To check  that the CCA is landed to the Engagement Centre search screen, after selecting the brand in the popup window", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=2)
	public void TC_142(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(searchPage.uielement.verifyPageElements(Arrays.asList("txtPolicyNumber"), searchPage),
					"Engagement Centre Search Page is verified", "Engagement Centre Search Page is not verified",
	...
	@Test(description = "To check that the \"All Brands\" option is NOT listed in \"My Brands\" pop up window, when click on \"Take Call/Make Call\" action button", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=3)
	public void TC_144_146(String browser) throws Exception {
		...
		try {
			...
			// Click on Take Call link
			homePage.clickTakeCall(extentedReport);
			Log.softAssertThat(!homePage.uielement.verifyPageElements(Arrays.asList("btnAllBrands"), homePage),
	...
	@Test(description = "To check selected Brand screen will not save when CCA return back from Customer Dashboard to Agent Dashboard", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=4)
	...
	@Test(description = "To check for Insurer,brand name is displayed alongside the address of the customer in the customer search result page", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=5)
	public void TC_152_154(String browser) throws Exception {
		...
		try {
			...
			if (!searchPage.verifySearchResultCount("Results", extentedReport, true)) {
				...
				Log.softAssertThat(
						custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"),
								custdashboardpage),
						"Custdashboardpage is Verified", "Custdashboardpage not Verified", driver, extentedReport,
						true);
				custdashboardpage.clickComplete(extentedReport);
			...
			Log.softAssertThat(searchPage.isSearchTitleAvaialble(extentedReport),
					"Brand column is displayed in search results", "Brand column is not displayed in search results",
	...
	@Test(description = "To check CCA can able to create New Customer after selecting brand from \"My Brand\" list", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=6)
	public void TC_157_158(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(
					...
					"Selected brand is not displayed in customer dashboard page", driver, extentedReport, true);
			Log.testCaseResult(extentedReport);
	...
	@Test(description = "To check that the CCA is prompted to select again the brand after clicked \"Admin\" action and create new customer", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=7)
	public void TC_160_161_162(String browser) throws Exception {
		...
		description = testData.get("Description");
		brandName = testData.get("Brand Name");
		String warningMsg = "You must select a brand before you can proceed with creating a new customer";
		...
		try {
			...
			Log.softAssertThat(searchPage.uielement.verifyPageElements(Arrays.asList("modalMyBrands"), searchPage),
					"System is prompted the My brands popup to select the brand",
					"System is not prompted the My brands popup to select the brand", driver, extentedReport, true);
			...
			Log.softAssertThat(searchPage.verifyWarningMsgForBrand(warningMsg, extentedReport, true),
					...
					"Waring message is not displayed in My Brands popup.Message: " + warningMsg, driver, extentedReport,
					true);
			Log.message("<b>Verification for TC_162</b>", extentedReport);
			searchPage.clickBrandPopupCancelBtn(extentedReport, true);
			Log.softAssertThat(searchPage.uielement.verifyPageElements(Arrays.asList("txtPolicyNumber"), searchPage),
					"Engagement Centre Search Page Verified", "Engagement Centre Search Page not Verified", driver,
					extentedReport, true);
			Log.testCaseResult(extentedReport);
	...
	@Test(description = "To check that All Brand option will displayed in the My brand pop up if only two Brand has been configured in the Business Unit Profile", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=1)
	public void TC_139c_142a(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(searchPage.verifyBrandNameForAllContacts(brandName, extentedReport, true),
					...
					"All searched contacts are not respect to " + brandName + " brand", driver, extentedReport, true);
			Log.testCaseResult(extentedReport);
	...
	@Test(description = "To check that only two brand displayed in the My brand pop up which has configured in the Business Unit Profile", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider",priority=0)
	public void TC_139b(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(homePage.verifyBrandsInPopup(brandList[0], extentedReport, true),
					...
					brandList[0] + " brand is not displayed in My Brands popup", driver, extentedReport, true);
