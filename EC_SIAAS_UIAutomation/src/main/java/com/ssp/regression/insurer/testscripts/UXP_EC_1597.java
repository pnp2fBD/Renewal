...
public class UXP_EC_1597 extends BaseTest {
	...
	@Test(description = "Create NB policy validate collapse / expand functionality of each tabs in customer dashboard page", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Check the policy bar on customer dashboard page", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_003(String browser) throws Exception {
		...
		try {
			...
			custdashboardpage.verify_PolicyTab(policyDetails.get("PolicyNo").toString(), extentedReport, true);
	...
	@Test(description = "Check whether the first menu option in the 'policy information' section is Policy details when a policy is selected", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Check whether the navigation button reads 'Policy Details' and show as selected even when the 'policy cover' menu option is selected and check whether the user is able to navigate back to 'Policy Details' section by selecting the menu option 'policy details'", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_019_021_022(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(
					custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"), custdashboardpage),
					"Customer Dashboardpage verified", "Customer Dashboardpage not verified", driver, extentedReport, true);
			...
			//custdashboardpage.SelectMenu_from_policyDetails_tab("Policy Details", extentedReport, true);
			...
			if (GenericUtils.compareTwoHashMap(policyDetails,
				...
				Log.pass("Both policy details tab and view policy quotes has the same policy references", driver,
						extentedReport, true);
			else
				Log.fail("Policy details tab and view policy quotes failed to have the same policy references", driver,
						extentedReport, true);
	...
	public void TC_025(String browser) throws Exception {
		...
		try {
			 ...
			 * // Click on Take Call link
			 * homePage.clickTakeCall(extentedReport);
			 * homePage.clickMyBrands(brandname, extentedReport); SearchPage
			 ...
			 * 
			 * // Enter Customer Details searchPage.clickCreateCustomer(true,
			 * extentedReport); searchPage.enterCustomerDetails(testData, true,
			 * extentedReport);
			 * 
			 * // Confirm customer details and create customer CustDashboardPage
			 * custdashboardpage = searchPage.confirmCreateCustomer(true,
			 * extentedReport); Log.softAssertThat(
			 * custdashboardpage.uielement.verifyPageElements(Arrays.asList(
			 * "lnkEditDetails"), custdashboardpage),
			 * "custdashboardpage Verified", "custdashboardpage not Verified",
			 * driver, extentedReport, true);
			 * Log.softAssertThat(custdashboardpage.verifyContactName(
			 * CustomerName, extentedReport, true),
			 * "Verified Searched FirstName and LastName on Customer Dashboard",
			 * "Not Verified Customer Details on Dashboard", driver,
			 * extentedReport, true);
			 * 
			 * // Create New Quote custdashboardpage.clickNewQuote(true,
			 * extentedReport); custdashboardpage.enterQuoteDetails(testData,
			 * true, extentedReport); NewQuotePage newquotepage =
			 * custdashboardpage.clickContinueQuote(true, extentedReport);
			 * 
			 * // Create New Quote
			 * Log.softAssertThat(newquotepage.verifyNewQuotePage(true,
			 * extentedReport), "Verified NewQuotePage ",
			 * "Not verified NewQuotePage", driver, extentedReport, true);
			 * 
			 * newquotepage.enterPolicyDetails(testData, true, extentedReport);
			 * newquotepage.clickNextOne(extentedReport);
			 * newquotepage.enterCustomerDetails(testData, true, Corecover,
			 * extentedReport); newquotepage.clickNextTwo(extentedReport);
			 * newquotepage.clickView(extentedReport);
			 * newquotepage.clickAgree(extentedReport); // Step-8: Get and Buy
			 ...
			 * (policyDetails.get("Status").toString().equalsIgnoreCase(
			 * "Accepted")) {
			 * 
			 * Log.pass("NB Policy Created Successfully in accepted status",
			 * driver, extentedReport, true); } else {
			 * Log.fail("Failed - NB Policy Created in " +
			 * policyDetails.get("Status").toString() + " status", driver,
			 * extentedReport, true); }
			 * 
			 ...
			 * custdashboardpage.clickNewQuote(true, extentedReport);
			 * custdashboardpage.enterQuoteDetails(testData, true,
			 * extentedReport); newquotepage =
			 * custdashboardpage.clickContinueQuote(true, extentedReport);
			 * 
			 * // Create New Quote
			 * Log.softAssertThat(newquotepage.verifyNewQuotePage(true,
			...
			Log.pass("Navigated to Cutomer Dashboard after selecting policy from search page", driver, extentedReport,
					true);
	...
	// TC_24
	@Test(description = "Check whether the 'View Policy Notes' section does not have a heading 'Policy Notes' below the bar", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Check whether 'View more Quotes and Policies' tables are hoverable and able to select the row", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
