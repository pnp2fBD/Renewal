...
public class UXP_EC_1600 extends BaseTest {
	...
	@Test(description = "Check that the Policies and Quotes heading which appears above the grid is renamed to Select a policy/quote to verify", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_624(String browser) throws Exception {
		...
		try {
			...
			if (policyDetails.get("Status").toString().equalsIgnoreCase("Accepted")) {
				...
				Log.softAssertThat(homePage.verifyHomePage(), "Successfully navigated to Home Page", "Failed to navigate to Home Page",
						driver, extentedReport, true);
	...
	@Test(description = "Check that the message 'There are no Quotes or Policies associated with this Contact' is displayed when the selected user does not have any policy/quote", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_635(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(
					custdashboardpage.uielement.verifyPageElements(Arrays.asList("lnkEditDetails"), custdashboardpage),
					"Customer Dashboard page Verified", "Customer Dashboard page not verified", driver, extentedReport, true);
	...
	@Test(description = "Verify the display of 'Proceed' button & behaviour of proceed button", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Check that the \"Proceed\" button is removed, if the selected contact has policy/quote and user should able to navigate to customer dashboard when clicked on the policy", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Check that the search page has caret symbold faced upwards when section is expanded", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_628(String browser) throws Exception {
		...
		try {
			...
			Log.softAssertThat(searchPage.verifysearch_CollapseBtn(),
					...
					"Search Page caret symbol was not Verified or not found on search page", driver, extentedReport,
					true);
			Log.testCaseResult(extentedReport);
		} catch (Exception e) {
			Log.exception(e, driver, extentedReport);
	...
	@Test(description = "Verify the display of the selected contacts in search result , it should move to top of search list when clicked", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	...
	@Test(description = "Check that the search results are in alphabetical order", dataProviderClass = DataProviderUtils.class, dataProvider = "ssBVTDataProvider")
	public void TC_632(String browser) throws Exception {
		...
		try {
			...
			for(int i = 0;  i<=2;i++) {			
				// Click on Take Call link
				if (i>0) testData = getTestData(tcId+"_"+i);
				homePage.clickTakeCall(extentedReport);
				homePage.clickMyBrands(brandname, extentedReport);
				SearchPage searchPage = new SearchPage(driver, extentedReport).get();
				Log.softAssertThat(searchPage.uielement.verifyPageElements(Arrays.asList("txtPolicyNumber"), searchPage),
