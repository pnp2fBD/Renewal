...
public class BuyQuotePage extends LoadableComponent<BuyQuotePage> {
	...
	@FindBy(css = ".adjustwidth.prog_Active span")
	...
	// Data capture page
	@FindBy(css = "input[name='C2__CONTACTCENTRE[1].COMMON[1].VARIATIONDESCRIPTION[1].QUOTEDESCRIPTION']")
	 ...
	 *            - Reason name to verify extendReport Screenshot
	 * @throws Exception
	 * @return boolean
	 * 
	 */
	public boolean verify_AdjReason_Selected(String optionToverify, ExtentTest extendReport, boolean Screenshot)
			throws Exception {
	 ...
	 *
	 * @param paymentmethod
	 *            : String extendReport Screenshot
	 * @throws Exception
	 */
	public void selectPaymentMethod(String paymentMethod, ExtentTest extendReport, boolean Screenshot)
		...
		try {
			...
			WaitUtils.waitForelementToBeClickable(driver, selectpaymentmethod, "Unable to find Payment method");
			selectpaymentmethod.sendKeys(paymentMethod);
			WaitUtils.waitForSpinner(driver);
	...
	public String enterChangePayorDetails(String lastName, String firstName, String postcode, ExtentTest extentedReport,
		...
		try {
			WaitUtils.waitForelementToBeClickable(driver, fldPayorLastName, "Last name field is not found");
	 ...
	 * 
	 */
	public void selectFirstBankDetail(ExtentTest extentedReport, boolean screenshot) throws Exception {
		try {
	...
	/**
	 * To click the navigation stepper on top of screen (to move across
	 * different tab)
	 * 
	 * @param tabToClick
	 *            as String
	...
	public void checkQuotePage_traverse(String tabToClick, ExtentTest extentedReport, boolean Screenshot)
			throws Exception {
		try {
			...
			case "dataCapture":
				...
				WaitUtils.waitForSpinner(driver);
				WaitUtils.waitForElementPresent(driver, txtYearofBuild, "Failed to navigate to data capture page");
			...
			case "acceptance":
				...
				WaitUtils.waitForSpinner(driver);
				WaitUtils.waitForElementPresent(driver, selectpaymentmethod, "Failed to navigate to acceptance page");
	...
	public boolean verifyDefaultTab(String tabName, ExtentTest extentedReport, boolean Screenshot) throws Exception {
		try {
	...
	public void clickTakePaymentBtn(ExtentTest extentedReport) throws Exception {
		try {
			...
			WaitUtils.waitForelementToBeClickable(driver, TakePayment, "Unable to find Takepayment Button");
			TakePayment.click();
			WaitUtils.waitForSpinner(driver);
	 ...
	 * 
	 */
	public void clickPayorConfirmationCheckbox(ExtentTest extentReport, boolean screenshot) throws Exception {
		try {
			chkPayorconfirmation.click();
			Log.message("Checkbox to confirm the change payor is clicked", driver, extentReport, screenshot);
	 ...
	 * 
	 */
	public void enterAccountNoAndSortCode(String AccountNumber, String SortCode, ExtentTest extentReport,
			boolean screenshot) throws Exception {
		try {
	 ...
	 * 
	 */
	public boolean VerifyAccountNoAndSortCode(String AccountNumber, String SortCode) throws Exception {
		try {
			boolean status = false;
			String newSortCode = txt_SortCode.getAttribute("value").replaceAll("-", "");

			if (txt_AccNumber.getAttribute("value").equals(AccountNumber) && newSortCode.equals(SortCode)) {
				status = true;
			}
	 ...
	 *             : Custom Exception Message
	 */
	public boolean verifyTandC_Edited(String policyToEdit, ExtentTest extentedReport, boolean Screenshot)
			throws Exception {
		try {
			...
			if (WaitUtils.waitForElement(driver, rowToClick.findElement(By.cssSelector(cssTandCTextArea)))) {
				...
				// Edit the text
				((JavascriptExecutor) driver).executeScript("arguments[0].innerText ='Testing'", txtAreaTandC);
	 ...
	 *             : Custom Exception Message
	 */
	public boolean clickDataCaptureandVerify(ExtentTest extentedReport, boolean Screenshot) throws Exception {
		try {
			if (WaitUtils.waitForElement(driver, btn_bcktoDatacapture, 2)) {
				btn_bcktoDatacapture.click();
				WaitUtils.waitForSpinner(driver);
				Log.message("Clicked on back to data capture button in price presentation page", extentedReport);
	...
	public void clickSavebuttonForRenewalQuoteinPPpage(String txtToenter, ExtentTest extentedReport) throws Exception {
		try {
			WaitUtils.waitForSpinner(driver);
	 ...
	 * 
	 * @param :btntoClickinAcptancePage
	 *            - enter the button user wanted to interact (Save / Lapse) as
	 *            String
	 * @throws Exception
	 *             : Custom Exception Message
	 * @param extentedReport
	...
	public boolean clicksave_LapsebuttoninAcceptancepage(String btnToClick,String quoteDesc,String btntoClickinAcptancePage, ExtentTest extentedReport)
			throws Exception {
		try {
			...
			case "lapse":
				...
				if (WaitUtils.waitForElement(driver, btnContinuewarning)
					...
					Log.message("Warning message, continue and cancel button were present in warning modal", driver,
							true);
			...
			//	if(WaitUtils.waitForElement(driver, radiobtnReviewed_acceptance) && WaitUtils.waitForElement(driver, radiobtnReviewreq_acceptance))
			//	Log.message("Both Reviewed and Review required button are visible on quote save modal", extentedReport);
				if(btnToClick.equalsIgnoreCase("Reviewed")) radiobtnReviewed_acceptance.click(); else radiobtnReviewreq_acceptance.click();
	...
	public boolean clicksave_ReviewedbuttoninAcceptancepage(String btntoClickinAcptancePage, ExtentTest extentedReport)
			throws Exception {
		try {
			boolean boolReturn = false;
			switch (btntoClickinAcptancePage.toLowerCase()) {
	...
	/**
	 * Description : click save button in data capture page and verify the quote
	 ...
	 * 
	 */
	public boolean enterQuoteDescVerifybuttons(String buttonToClick, ExtentTest extentReport, boolean Screenshot)
			throws Exception {
		try {
			...
			if (WaitUtils.waitForElement(driver, btnQuoteSave, 2)
				...
				Log.message("Button save and cancel present in the quote description dialog", driver, extentReport,
						true);
			else
	...
	public void clickCheckboxinPayment(ExtentTest extentedReport, boolean Screenshot) throws Exception {
		try {
			WaitUtils.waitForSpinner(driver);
	...
	public boolean clickCancelandClosebuttonppPageandVerify(String btnToClick, ExtentTest extentedReport,
			boolean Screenshot) throws Exception {
		try {
			boolean boolval = false;
			WaitUtils.waitForElementPresent(driver, btn_renewQuoteSave,
	 ...
	 * 
	 */
	public void addAccountDetail(HashMap<String, String> testdata, ExtentTest extentReport, boolean screenshot)
			throws Exception {
		try {
			clickAddAccountDetailButton(extentReport);
			enterAccountNoAndSortCode(testdata.get("Account Number").toString(), testdata.get("Sort Code").toString(),
					extentReport, false);

			String strAcctName = GenericUtils.getRandomCharacters("ALPHA", 5);
			enterAccountName(strAcctName, extentReport, screenshot);

			WaitUtils.waitForElementPresent(driver, but_AddAccSave, "Save button to add account is not found");
	 ...
	 * 
	 */
	public void enterAccountName(String accountName, ExtentTest extentReport, boolean screenshot) throws Exception {
		try {
	...
	public void clickAcceptBtn(ExtentTest extentedReport) throws Exception {
		try {
			...
			WaitUtils.waitForelementToBeClickable(driver, btnAcceptPayment, "Unable to find Accept Button");
			btnAcceptPayment.click();
			WaitUtils.waitForSpinner(driver);
	...
	public boolean clicksavebuttoninAcceptancepage(String description, ExtentTest extentedReport) throws Exception {
		boolean boolReturn = false;
		try {
	 ...
	 * 
	 */
	public void clickSaveOrLapse(String btnToClick, ExtentTest extentedReport, boolean Screenshot) throws Exception {
		try {
