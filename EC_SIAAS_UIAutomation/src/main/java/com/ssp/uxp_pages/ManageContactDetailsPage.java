...
public class ManageContactDetailsPage extends LoadableComponent<ManageContactDetailsPage> {
	...
	@FindBy(css = "#C2__C1__BUT_4E3BCB6B5201A369249686[title='Enter Address Manually']:not(.page-spinner)")
	...
	@FindBy(css = "#C2__C1__QUE_753975C04A0241C1630497[name*='ADDRESS[1].ADDRESSLIST']:not([disabled='disabled']) .list-group-item")
	...
	/**
	 * 
	 * Constructor class for Page Here we initializing the driver for page
	...
	public void checkRowEditBtn(ExtentTest extentedReport, boolean Screenshot) throws Exception {
		try {
			WebElement AddressActual = driver
					.findElement(By.cssSelector("tr#C2__C1__p0_TBL_753975C04A0241C1630453_R1>td"));
	...
	public boolean verifyMailingAddressChkBoxDisabled() throws Exception {
		boolean status = false;
		try {
			if (!chkboxMailingAddress.isEnabled()&& chkboxMailingAddress.getAttribute("disabled").contains("true") 
	...
	public void clickEnterManuallyLink(ExtentTest extentedReport, boolean screenshot) throws Exception {
		try {
	...
	public void enterReplaceAddressDetails(String fieldName, String fieldValue, ExtentTest extentedReport,
		...
		Log.message("Entered Address field " + fieldName + " : '" + fieldValue + "'", driver, extentedReport,
				screenshot);
	}
	...
	public void clickFindAddressButton(ExtentTest extentedReport, boolean screenshot) throws Exception {
		try {
	...
	public void clickFindAddressDetail(ExtentTest extentedReport, boolean screenshot) throws Exception {
		try {
	 ...
	 * @throws Exception
	 */
	public void clickAddNewAddressButton(ExtentTest extentedReport, boolean Screenshot) throws Exception {
		try {
			WaitUtils.waitForelementToBeClickable(driver, btnAddNewAddress, "Unable to find the AddNewAddress button");
	 ...
	 * @throws Exception
	 */
	public void enterNewAddressDetails(String postcode, String type, ExtentTest extentedReport, boolean Screenshot)
			throws Exception {
		try {
	 ...
	 * 
	 */
	public void enterAddress(String postCode, String type)
			throws Exception {
		try {
			...
			Thread.sleep(1000);
			((JavascriptExecutor) driver).executeScript("document.getElementById('C2__C1__QUE_753975C04A0241C1630594').value='"+ postCode + "';");
			Thread.sleep(2000);
