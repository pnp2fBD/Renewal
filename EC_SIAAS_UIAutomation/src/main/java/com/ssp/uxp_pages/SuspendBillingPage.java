...
public class SuspendBillingPage extends LoadableComponent<SuspendBillingPage> {
	 ...
	 * 
	 */
	public CustDashboardPage completeSuspendBilling(String reason, ExtentTest extentedReport, boolean screenshot)
			throws Exception {
		try {
	 ...
	 * 
	 */
	public void selectSuspendBillingReason(String reason, ExtentTest extentedReport, boolean screenshot)
			throws Exception {
		try {
			if (WaitUtils.waitForElement(driver, drpoptSuspendBillingReason)) {
				...
				Log.message("Reason for suspeded billing is selected - " + reason, driver, extentedReport, screenshot);
			} else {
	...
	public boolean isSuspendBillingSelected() throws Exception {
		try {
			...
			throw new Exception("Error while Verifying suspend billing is selected in buyQuote page: " + e);
		}
	}
	 ...
	 * 
	 */
	public void clickCustomerDashboardTab(ExtentTest extentedReport, boolean Screenshot) throws Exception {
		try {
			WaitUtils.waitForElement(driver, tabCustomerDashboard);
			tabCustomerDashboard.click();
			WaitUtils.waitForElement(driver, mdlReturnToDashboard);
			Log.message("CustomerDashboard tab is clicked to navigate to customer dashboard", driver, extentedReport,
					Screenshot);
		} catch (Exception e) {
