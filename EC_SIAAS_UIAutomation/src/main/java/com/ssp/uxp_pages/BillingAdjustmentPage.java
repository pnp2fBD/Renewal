...
public class BillingAdjustmentPage extends LoadableComponent<BillingAdjustmentPage> {
	...
	protected void isLoaded() {
		...
		if (isPageLoaded && !driver.getTitle().contains("Billing Adjustment")) {
			Log.fail("SSP Billing Adjustment Page did not open up. Site might be down.", driver,extentedReport);
		}
	...
	* 
	*/
	public void selectAdjustmentReason(String optionToClick, ExtentTest extendReport, boolean Screenshot)
			throws Exception {
		...
		try {
			...
			Log.message(optionToClick + " - Option is selected for adjustment reasoning ", driver, extendReport,
					Screenshot);
	...
	*/
	public void clickNext(ExtentTest extendReport) throws Exception {
		...
		try {
			if (WaitUtils.waitForElement(driver, btnNext)) {
				btnNext.click();
				Log.message("Next button is clicked after selecting adjustment reason", driver, extendReport);
			} else {
	...
	public String enterChangePayorDetails(String lastName, String firstName, String postcode, ExtentTest extentedReport,
		...
		try {
			...
			WaitUtils.waitForinvisiblityofElement(driver, 180, cssConfirmbutton, "Change payor search window did not close after 3min");
		} catch (Exception e) {
			throw new Exception("Unable to click Change Payor button : " + e);
		}
		return payorName;
	...
	public boolean VerifyAccountName(String AccountName, ExtentTest extentReport) throws Exception{
		try {
			...
			Log.message("Account name is displayed with the name - " + txt_AccName.getAttribute("value"), extentReport);
	...
	public void changeAddressOne(String Address1, ExtentTest extentReport, boolean Screenshot) throws Exception {
		try {
			...
			Log.message("Address1 detail is changed in payment panel as - " + Address1, driver, extentReport, Screenshot);
		} catch (Exception e) {
	 ...
	 * 
	 */
	public boolean verifyCustomerAddress(String[] address, ExtentTest extentReport, boolean Screenshot) throws Exception {
		try {
