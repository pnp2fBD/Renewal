...
public class SearchPage extends LoadableComponent<SearchPage> {
	...
	public String specific_policy_Cnt = "table.table.table-hover.table-striped[summary='PoliciesAndQuotes'] tbody";
	...
	public final String SEARCH_NOPOLICY_MSG = "There are no Quotes or Policies associated with this Contact.";
	...
	@FindBy(css = "#C2__p0_TBL_9C3BD3DB194DC120259738_R1")
	WebElement firstRow;

	@FindBy(css = "div#C2__FMT_1A3613CD19FD282C373423")
	WebElement table_policysearchsingleResult;

	@FindBy(css = "table.table.table-hover.table-striped[summary='PoliciesAndQuotes'] tbody td:nth-child(1) span")
	 ...
	 * 
	 * @return true if it confirms that the current page is Search page; false
	 *         otherwise.
	 * @throws Exception
	 */
	 ...
	 * 
	 * @param textToVerify
	 * @return true if the actual policy error message matches with the
	 ...
	 * 
	 * @param textToVerify
	 * @return true if the actual last name error message matches with the
	 *         expected; false otherwise.
	 * @throws Exception
	 */
	 ...
	 * 
	 * @param textToVerify
	 * @return true if the actual first name error message matches with the
	 *         expected; false otherwise.
	 * @throws Exception
	 */
	 ...
	 * 
	 * @param extentedReport
	 * @return the string representation of the last name value, or null if no
	 *         string representation is applicable.
	 * @throws Exception
	 */
	 ...
	 * @throws Exception
	 */
	public void enterPolicyNumber(String policynumber, ExtentTest extentedReport, boolean screenshot) throws Exception {
		try {
			WaitUtils.waitForElementPresent(driver, 180, txtPolicyNumber, "Timed out to find policy number text box.");
			txtPolicyNumber.clear();
			txtPolicyNumber.sendKeys(policynumber);
			WaitUtils.waitForSpinner(driver);
			Log.message("Entered Policy Number: " + policynumber.toString(), driver, extentedReport, screenshot);
		} catch (Exception e) {
	 ...
	 * @throws Exception
	 */
	public void searchValidPolicy(String policynumber, boolean screenShot, ExtentTest extentedReport) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void searchValidLastname(String lastname, boolean screenShot, ExtentTest extentedReport) throws Exception {
		Log.message("Search for a Valid policy Number", extentedReport);
	 ...
	 * @throws Exception
	 */
	public CustDashboardPage selectValidContact(String nameToSelect, boolean screenShot, ExtentTest extentedReport)
			throws Exception {
		CustDashboardPage objCustDashboardPage = null;
		Log.message("Selecting valid Contact", extentedReport);
		try {
			WaitUtils.waitForElementPresent(driver, 180, contact_Table, "Timed out to get search results");
			...
			if (isFound) {
				Log.message("Search results have the contact with name '" + nameToSelect + "'.", extentedReport);
				RowVal.click();
				WaitUtils.waitForSpinner(driver);
				...
				throw new Exception("Search results does not have the contact with name '" + nameToSelect + "'.");
			}
		} catch (Exception e) {
	...
	public boolean isSearchTitleAvaialble(ExtentTest extentedReport) {
		...
		if (result) {
			if (lblSearchResultTitleContactName.getText().contains("Contact Name")
				...
				Log.message("Title available : Contact Name, Date of Birth, Address, Brand", extentedReport);
				result = true;
			} else {
				Log.message("One or more of title not available : Contact Name, Date of Birth, Address, Brand",
						extentedReport);
				result = false;
			}
		} else {
	 ...
	 * 
	 */
	public CustDashboardPage selectContactAndMoveToCustDashboard(String NameToSelect, boolean screenShot,
		...
		try {
			...
			(new WebDriverWait(driver, 40).pollingEvery(200, TimeUnit.MILLISECONDS)
					...
					.withMessage("Unable to get search results")).until(ExpectedConditions.visibilityOf(contact_Table));
	...
	public CustDashboardPage selectPolicyAndMoveToCustDashboard(String NameToSelect, boolean screenShot,
			ExtentTest extentedReport) throws Exception {
		Log.message("Selecting contact");
		try {
			WaitUtils.waitForSpinner(driver);
	 ...
	 * Writes the given customer details in the 'Create New Customer' form.<br/>
	 * <br/>
	 * The vararg '<i>shouldForceRandomNameCreation</i>' of the method is used to
	 * tell whether the random name (both first name and last name) should be
	 * entered in form despite the name value given in <i>testData<i/>.
	 * 
	 * @param testData
	 * @param screenShot
	 * @param extendReport
	 * @param shouldForceRandomNameCreation
	 * @return the <code>HashMap</code> representation of testData, or null if no
	 *         <code>HashMap</code> representation is applicable
	 * @throws Exception
	 */
	/*public HashMap<String, String> enterJointCustomerDetails(HashMap<String, String> testData, boolean screenShot,
			ExtentTest extendReport, boolean... shouldForceRandomNameCreation) throws Exception {
		try {
			WaitUtils.waitForElementPresent(driver, Ele_engagement_Centre_tabs,
					"Unable to find Perosnal/corporate contact tabs");
			...
			boolean forceRandomValueFlag = shouldForceRandomNameCreation.length > 0 ? shouldForceRandomNameCreation[0]
					: false;
			String title1 = testData.get("Title").toString();
			String firstName1 = testData.get("First Name").toString();
			String lastName1 = testData.get("Last Name").toString();
			String dob1 = testData.get("Date of Birth").toString();
			String gender1 = testData.get("Gender").toString();
			String postcode1 = testData.get("Post Code").toString();
			String homephone1 = testData.get("Home Phone").toString();
			String email1 = testData.get("Email").toString();

			Log.message("Entering Customer details");
			selectTitle(title1);
			Log.message("Title : " + title1);

			if (forceRandomValueFlag || firstName1.equals(""))
				firstName1 = GenericUtils.getRandomCharacters("alpha", 5);

			entercn_firstName(firstName1);
			testData.put("First Name", firstName1);
			Log.message("First Name : " + firstName1, extendReport);

			if (forceRandomValueFlag || lastName1.equals(""))
				lastName1 = GenericUtils.getRandomCharacters("alpha", 4);

			entercn_LastName(lastName1);
			testData.put("Last Name", lastName1);
			Log.message("Last Name : " + lastName1, extendReport);

			entercn_DateOfBirth(dob1);
			Log.message("Date of Birth : " + dob1, extendReport);

			selectGender(gender1);
			Log.message("Gender : " + gender1, extendReport);

			entercn_PostCode(postcode1);
			Log.message("Post Code : " + postcode1, extendReport);
			...
			testData.put("Address2", getAddress2());
			testData.put("Address3", getAddress3());
			testData.put("Postcode", postcode1);
			
			entercn_HomePhone(homephone1);
			Log.message("Home Phone : " + homephone1, extendReport);

			if (!email1.contains("@"))
				email1 = testData.get("Email") + DateTimeUtility.getCurrentDateAndTime() + "SIAAS@ectest.com";

			cn_Email.clear();
			cn_Email.sendKeys(email1);

			Log.message("Email : " + email1, extendReport);
			testData.put("Email", email1);
	...
	public ArrayList<String> enterJointCustomerDetails(HashMap<String, String> testData, boolean screenShot,
			ExtentTest extendReport, boolean... shouldForceRandomNameCreation) throws Exception {
	...
	public CustDashboardPage confirmCreateCustomer(boolean screenShot, ExtentTest extentedReport) throws Exception {
		WaitUtils.waitForSpinner(driver);
		try {
	...
	public void searchbybrandName(String NameToSelect, boolean screenShot, ExtentTest extentedReport) throws Exception {
	 ...
	 * 
	 */
	public void selectPolicyId(String policyNumber, ExtentTest extentedReport, boolean screenShot) throws Exception {
		try {
//			(new WebDriverWait(driver, 20).pollingEvery(200, TimeUnit.MILLISECONDS)
//					.ignoring(NoSuchElementException.class, StaleElementReferenceException.class)
//					.withMessage("Unable to get search results"))
//							.until(ExpectedConditions.visibilityOf(lstSearchResults));
	...
	/**
	 * 
	 * Description : Search any policy number in search page, and Select the
	 * policy to open policy details page
	 * 
	 * 
	 * @param policyNumber
	 *            - Policy number, wants to search
	 * @param extentedReport
	 * @param screenShot
	 * @throws Exception
	 *             as custom Exception Message
	 * 
	 */
	public void searchAndSelectPolicyId(String policyNumber, ExtentTest extentedReport, boolean screenShot)
			throws Exception {
		try {
			enterPolicyNumber(policyNumber, extentedReport, screenShot);
			clickSearch(extentedReport);
	...
	public CustDashboardPage selectPolicy_from_SearchPage(boolean screenShot, ExtentTest extentedReport)
			throws Exception {
		...
		try {
			...
			Log.message("Sucessfully searched policy : " + txt_PolicyNo.getText() + " through quick search",
					extentedReport);
			RowVal.click();
			WaitUtils.waitForSpinner(driver);
			Log.message("Clicked on newly created policy in search policy", driver, extentedReport, screenShot);
	...
	/**
	 * 
	 * Description : After Search, verify search list heading when policy/quote
	 * associate to a contact
	 * 
	 * Return : boolean
	 * 
	 * @param screenShot
	...
	public boolean verifyHeading(boolean Screenshot) throws Exception {
		return GenericUtils.verifyWebElementTextEquals(txt_searchresHeading, "Select a policy/quote to verify");

	}

	/**
	 * 
	 * Description : After Search, verify string when no search details
	 * displayed
	 * 
	 * Return : boolean
	 * 
	...
	public boolean verify_stringonSearchPage(ExtentTest extentedReport, boolean Screenshot) throws Exception {
	 ...
	 * 
	 */
	public CustDashboardPage click_proceedButton(ExtentTest extentedReport, boolean Screenshot) throws Exception {
		WaitUtils.waitForElementPresent(driver, 180, btn_proceed, "Unable to find proceed button on search page");
		try {
			btn_proceed.click();
			WaitUtils.waitForSpinner(driver);
			...
			throw new Exception("Failed to locate proceed button on search page. Exception occured : " + e);
	...
	/**
	 * 
	 * Description : After Search, verify the presence of collapse button on
	 * search result
	 * 
	 * Return : boolean
	 * 
	...
	/**
	 * 
	 * Description : After Search, verify the search result by clicking each row
	 * , validate whether the result moves to top
	 * 
	 * Return : boolean
	 * 
	 * @param extentedReport
	...
	public boolean verifysearch_Results(ExtentTest extentedReport, boolean screenShot) throws Exception {
		...
		try {
			...
			for (int i = 1; i <= ContactRows.size(); i++) {
				WebElement ele_contactname = driver.findElement(By.cssSelector(contact_searchRows + i + "']"));
			...
			Log.message("Search results are clickable and moves to top of search list when clicked", driver,
					extentedReport, true);
			return srchResult;
		}
	...
	/**
	 * 
	 * Description : After Search, verify the search result are in alphabetical
	 * order
	 * 
	 * @param extentedReport
	 *            Return : boolean
	...
	public boolean verify_results_Alphabeticalorder(ExtentTest extentedReport) throws Exception {
		try {
			...
			throw new Exception("Failed to verify the contact lists in alphabetical order, exception occured: " + e);
		}
	}
	...
	public void selectcontactFromList(String NameToSelect, ExtentTest extentedReport, boolean screenShot)
			throws Exception {
		...
		try {
			...
			if (boolVal) {
				...
				WaitUtils.waitForSpinner(driver);
				Log.message("Selecting the contact " + NameToSelect + " from search result", driver, extentedReport,
						true);
			}
	...
	public void createCorporateContact(HashMap<String, String> testdata, ExtentTest extentReport, boolean Screenshot)
			throws Exception {
	 ...
	 *             : Custom Exception Message
	 */
	public void enterTradeDetails(HashMap<String, String> testdata, ExtentTest extentReport) throws Exception {
		try {
	 ...
	 *            :hashMap
	 */
	public void enterAuthContactDetails(HashMap<?, ?> testdata, ExtentTest extentReport) throws Exception {
		try {
	 ...
	 *            :hashMap
	 */
	public void enterAuthContactphoneDetails(HashMap<?, ?> testdata, ExtentTest extentReport) throws Exception {
		try {
	 ...
	 *            :String
	 */
	public void enterTradeName(HashMap<String, String> testdata, ExtentTest extentReport) throws Exception {
		try {
	...
	/**
	 * To click confirm button in corporate contact window Returns -
	 * customerdashboard apge
	 * 
	 * @return custDashboardPage
	 * @throws Exception
	 * @param extentedReport
	 ...
	 *            :boolean
	 */
	public CustDashboardPage confirmCreateCustomer_CC(boolean screenShot, ExtentTest extentedReport) throws Exception {
		WaitUtils.waitForSpinner(driver);
		try {
			...
			if (WaitUtils.waitForElement(driver, but_Proceed)) {
				...
				for (int loopcount = 0; loopcount < totalRowSize; loopcount++) {
					...
					if (displayedContactName.contains(NameToSelect)) {
						Log.message(NameToSelect + " - Contact name is available in Contact table after search", driver,
	 ...
	 * 
	 */
	public void searchAndSelectPolicyId(String policyNumber, ExtentTest extentedReport) throws Exception {
		try {
	...
	/**
	 * 
	 * Description : Enter any policy number in search page, and Select the
	 * policy to open policy details page
	 * 
	 * @param policyNumber
	 *            - Policy number, wants to search
	 ...
	 *            - Policy / contact as String
	 * @param policyNo
	 *            - Provide the policy no, if policy need to be searched
	 * @param cntctName
	 *            - contact name to be verified after search
	 * @param screenShot
	 *            boolean
	 * @param extentedReport
	 * @param policyStatus[Optional]
	 *            to search for policy status and click
	 * @return CustDashboardPage
	 * 
	 * @throws Exception
	 *             : Custom Exception Message
	...
	public CustDashboardPage clickSearchedContactPolicy(String searchCriteria, String policyNo, String cntctName,
			boolean screenShot, ExtentTest extentedReport, String... policyStatus) throws Exception {
		Log.message("Selecting Valid Contact");
		try {
			...
			case "contact":
				...
				if (WaitUtils.waitForElement(driver, table_policysearchsingleResult, 1)
						&& !GenericUtils.verifyWebElementTextContains(txt_withnopolicies, SEARCH_NOPOLICY_MSG)) {
					boolean boolstatus = false;
					if (GenericUtils.verifyWebElementTextContains(txtcontactName, cntctName))
						Log.message("Contact search using first and last name was successful", extentedReport);
					else
					...
					if (lstOfpolicy.size() > 1 && policyStatus.length > 0) {
						...
						for (int i = 1; i <= lstOfpolicy.size(); i++) {
							WebElement ele_polStatus = driver
									.findElement(By.cssSelector(lstofpolStatus + i + " td:nth-child(5)"));
							String displayedpolStatus = ele_polStatus.getText();
							if (displayedpolStatus.equalsIgnoreCase(reqStatus)) {
								Log.message(cntctName
				...
				else if (WaitUtils.waitForElement(driver, but_Proceed, 1)
						&& GenericUtils.verifyWebElementTextContains(txt_withnopolicies, SEARCH_NOPOLICY_MSG)) {
					but_Proceed.sendKeys(Keys.ENTER);
				...
				// quits the test case execution
					...
					if (ContactRows.size() > 1)
						Log.message("Search results displayed too many results with same last and first name", driver,
								extentedReport, true);
	 ...
	 * 
	 */
	public boolean verifyPolicyTableInSearchResult(ExtentTest extentedReport, boolean screenShot) throws Exception {
	 ...
	 * 
	 */
	public boolean verifySearchResultCount(String count, ExtentTest extentedReport, boolean screenShot) {
		boolean status = false;
	 ...
	 * 
	 */
	public boolean verifyWarningMsgForBrand(String actualMessage, ExtentTest extentedReport, boolean screenShot) {
		boolean status = false;
	 ...
	 * 
	 */
	public boolean verifyBrandNameForAllContacts(String actualBrandName, ExtentTest extentedReport, boolean screenShot)
			throws Exception {
		boolean status = false;
		try {
			WaitUtils.waitForSpinner(driver);
	 ...
	 * 
	 */
	public boolean verify_reviewStatus(String expectedStatus, ExtentTest extentedReport, boolean screenshot)
			throws Exception {
		try {
