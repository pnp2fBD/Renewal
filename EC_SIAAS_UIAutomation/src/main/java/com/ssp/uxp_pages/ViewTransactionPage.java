...
public class ViewTransactionPage extends LoadableComponent <ViewTransactionPage> {
	...
	protected void load() {
		...
		(new WebDriverWait(driver, 20).pollingEvery(200, TimeUnit.MILLISECONDS).ignoring(NoSuchElementException.class, StaleElementReferenceException.class).withMessage( "View Transaction page not loaded properly")).until(ExpectedConditions.visibilityOf(lnkBacktoDashboard));
	}
	...
	* 
	*/
	public void selectTab(String tabToClick, ExtentTest extendReport, boolean Screenshot) throws Exception {
		try {			
			switch(tabToClick) {
				...
				case "Data Caputure":
					...
					Log.message("Data Caputure tab is clicked in view transaction page", driver, extendReport, Screenshot);
					break;
					
				case "Price Presentation":
					WaitUtils.waitForElement(driver, tabPricePresentation);
					tabPricePresentation.click();
					Log.message("Price Presentation tab is clicked in view transaction page", driver, extendReport, Screenshot);
					break;
					
				case "Declaration":
					WaitUtils.waitForElement(driver, tabPricePresentation);
					tabDeclaration.click();
					Log.message("Declaration tab is clicked in view transaction page", driver, extendReport, Screenshot);
					break;	
			}
	...
	*/
	public boolean isViewBtnEnabled() throws Exception {
		try {
			...
			throw new Exception("Error while checking whether view button is enabled in view transaction page : " + e);
		}
